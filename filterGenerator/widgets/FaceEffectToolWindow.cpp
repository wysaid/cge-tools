...
FaceSpriteWidget::FaceSpriteWidget(QWidget* parent, QOpenGLWidget* shareWidget) :
    QOpenGLWidget(parent), m_sprite(nullptr), m_pointDrawer(nullptr)
{
    ...
    queueEvent([&, jsonConfig, callback]() {
        ...
        if (!elements.isEmpty())
        {
            ...
            for (const auto& elem : elements)
            {
                ...
                QImage&& img =
                    QImage(imageFilePath).convertToFormat(QImage::Format_RGBA8888_Premultiplied, Qt::ColorOnly);
                ...
                tmpFrame.textureID =
                    cgeGenTextureWithBuffer(img.bits(), img.width(), img.height(), GL_RGBA, GL_UNSIGNED_BYTE);
        ...
        if (m_pointDrawer == nullptr)
        {
            ...
            if (m_pointDrawer == nullptr) return;
...
QString FaceSpriteWidget::genConfig()
{
    ...
    if (!spriteConfig.isObject())
    {
        ...
        if (!spriteConfig.isObject()) return "";
    }
...
void FaceSpriteWidget::jumpFrame(float progress)
{
    ...
    queueEvent([=]() { m_sprite->jumpToFrame(progress * (m_sprite->totalFrames() - 1)); });
...
// FaceEffectToolMarkWidgetCanvas::FaceEffectToolMarkWidgetCanvas(QWidget* parent, FaceSpriteWidget* shareWidget) :
// QGLWidget(parent, shareWidget), m_spriteWidget(shareWidget)
// {
//
// };
//
...
FaceEffectToolMarkWidget::FaceEffectToolMarkWidget(
    FaceEffectToolWindow* parent, const std::function<void(std::vector<FaceSpriteWidget::FacePoints>*)>& func) :
    MenuDialogCommon(parent), m_callback(func), m_currentTarget(0)
{
...
void FaceEffectToolMarkWidget::keyReleaseEvent(QKeyEvent* e)
{
    ...
    {
        case Qt::Key_Right:
            if (sprite->currentFrame() != sprite->totalFrames() - 1) sprite->nextFrame(1);
            //		m_currentTarget = 0;
            //        m_currentTarget += 3;
            break;
        case Qt::Key_Left:
            if (sprite->currentFrame() != 0) sprite->jumpToFrame(sprite->currentFrame() - 1);
            //		m_currentTarget = 0;
            //        m_currentTarget -= 3;
            break;
        case Qt::Key_0:
...
void FaceEffectToolMarkWidget::paintEvent(QPaintEvent*)
{
    ...
    painter.drawText(
        0, 30, QString("Current frame: %1, Selected keypoint: %2").arg(sprite->currentFrame()).arg(m_currentTarget));
...
FaceEffectToolWindow::FaceEffectToolWindow(const std::function<void()>& func, MainWindow* mainwindow) :
    QWidget(nullptr), m_destructFunc(func)
{
    m_ui.setupUi(this);
...
{
    if (m_spriteWidget->sprite() == nullptr) return;

    m_spriteWidget->pausePlay();
    m_spriteWidget->hide();

    FaceEffectToolMarkWidget* widget =
        new FaceEffectToolMarkWidget(this, [&](std::vector<FaceSpriteWidget::FacePoints>* pnts) {
