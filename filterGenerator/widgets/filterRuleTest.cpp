...
FilterRuleTest::FilterRuleTest(QWidget* parent) : QMainWindow(parent)
{
    m_ui.setupUi(this);
    initWidgets();
...
{
    auto sz = e->size();
    m_ui.ruleEdit->setFixedSize(m_ui.ruleEdit->width(),
                                sz.height() - m_ui.inputBox->height() - m_ui.resBox->height() - m_ui.funcBox->height() -
                                    35);
...
void FilterRuleTest::testRule()
{
    ...
    if (!filter->initWithEffectString(filterRule.toLocal8Bit()))
    {
        QMessageBox::critical(
            this, "Failed to initialize filter!",
            "Invalid rule, or unable to create Filter! See the red text in the console window for details!");
        delete filter;
        return;
    }
...
{
    QString&& name = QFileDialog::getSaveFileName(this, QStringLiteral("Save The Image"), QDir::currentPath(),
                                                  "Images (*.png *.jpg *.xpm)");
    if (name.size() == 0)
    {
        return;
    ...
    if (QFile::exists(name))
    {
        if (QMessageBox::question(this, QStringLiteral("File conflict!"),
                                  QString("%1 already exists. Overwrite?").arg(name), QMessageBox::Yes,
...
void FilterRuleTest::batchEffect()
{
    ...
    CGE_LOG_INFO(
        "Please select a location to save the processed files. The output filename will be similar to the source filename.\n");

    QString&& savePath =
        QFileDialog::getExistingDirectory(this, "Please select a directory to store the output files!");
    if (savePath.isEmpty())
    {
    ...
    for (auto& name : filenames)
    {
        ...
        if (index >= 0) name.remove(0, index + 1);
