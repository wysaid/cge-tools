...
CGE::SharedTexture genSharedTextureWidthImageName(const char* filename, GLenum filter)
{
	...
	GLuint texID = cgeGenTextureWithBuffer(img.bits(), img.width(), img.height(), GL_RGBA, GL_UNSIGNED_BYTE, 4, 0, filter);
...
bool CanvasWidget::openImage(const QString& filename)
{
	...
	if (img.isNull())
	{
		QMessageBox::information(this, "Cannot Open Image", QString("%1 cannot be recognized as a valid supported image format and cannot be opened. Please select another file!").arg(filename));
		return false;
	}
...
bool CanvasWidget::openImage(QImage& img)
{
	...
	{
		CGE_LOG_ERROR("%s", (const char*)QString("The selected image size is %1 * %2, totaling %3M pixels. It is too large for real-time processing. To prevent excessive memory usage and maintain adjustment speed, the image will be cropped to within %4M pixels.\n").arg(img.width()).arg(img.height()).arg(img.width() * img.height() / 1e6f).arg(sMaxImageSize / 1e6f).toLocal8Bit());
