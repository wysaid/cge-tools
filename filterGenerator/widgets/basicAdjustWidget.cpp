...
void ContrastAdjustWidget::setContrast(int n)
{
    ...
    m_intensity = n / 100.0f;
    if (m_intensity > 1.0f) m_intensity = 1.0f + (m_intensity - 1.0f) * 4.0f;
...
void BlurSharpenAdjustWidget::setIntensity(int n)
{
    if (!checkFilter()) return;
    m_intensity = n / 100.0f;
    if (m_intensity > 1.0f) m_intensity = (m_intensity - 1.0f) * 9.0f + 1.0f;
...
{
    if (m_ui.hueEdit->text().size() == 0 || m_ui.saturationEdit->text().size() == 0 ||
        m_ui.luminanceEdit->text().size() == 0)
    {
        FILTER_COMMON_ERROR_MSG;
        return;
    }
...
HSVAdjustWidget::HSVAdjustWidget(MainWindow* mainWindow, QWidget* parent) : FilterAdjustWidgetBase(mainWindow, parent)
{
    initHSV();
}
...
void HSVAdjustWidget::initHSV()
{
    ...
    QLineEdit* edits[] = {m_ui.redEdit,    m_ui.greenEdit,   m_ui.blueEdit,
                          m_ui.yellowEdit, m_ui.magentaEdit, m_ui.cyanEdit};
...
bool HSVAdjustWidget::_preview(FormatContent* fmt)
{
    ...
    if (fmt != nullptr &&
        (red == 0.0f && green == 0.0f && blue == 0.0f && yellow == 0.0f && magenta == 0.0f && cyan == 0.0f))
    {
        QMessageBox::warning(this, "Invalid parameters!", "Please enter valid parameters!");
        return false;
    }

    if (!m_ui.redEdit->text().isEmpty()) m_ui.redEdit->setText(QStringLiteral("%1").arg(red));
    if (!m_ui.greenEdit->text().isEmpty()) m_ui.greenEdit->setText(QStringLiteral("%1").arg(green));
    if (!m_ui.blueEdit->text().isEmpty()) m_ui.blueEdit->setText(QStringLiteral("%1").arg(blue));
    if (!m_ui.yellowEdit->text().isEmpty()) m_ui.yellowEdit->setText(QStringLiteral("%1").arg(yellow));
    if (!m_ui.magentaEdit->text().isEmpty()) m_ui.magentaEdit->setText(QStringLiteral("%1").arg(magenta));
    if (!m_ui.cyanEdit->text().isEmpty()) m_ui.cyanEdit->setText(QStringLiteral("%1").arg(cyan));
...
void WhiteBalanceAdjustWidget::setTint(int n)
{
    if (!checkFilter()) return;
    m_tint = n / 100.0f;
    if (m_tint > 1.0f) m_tint = (m_tint - 1.0f) * 4.0f + 1.0f;
...
{
    if (m_ui.redShiftEdit->text().size() == 0 && m_ui.greenShiftEdit->text().size() == 0 &&
        m_ui.blueShiftEdit->text().size() == 0)
    {
        FILTER_COMMON_ERROR_MSG;
        return;
    }
...
void ColorLevelAdjustWidget::apply()
{
    ...
    if (m_dark >= m_light)
    {
        QMessageBox::information(
            m_mainWindow, QStringLiteral("Invalid Arguments"),
            "The 'dark' parameter of color level should not be greater than the 'light' parameter. Please correct it.");
        return;
    }
...
bool SelectiveColorAdjustWidget::_preview(FormatContent* pContent)
{
    ...
    for (int i = 0; i != 9; ++i)
    {
        ...
        if (cyan != 0.0f || magenta != 0.0f || yellow != 0.0f || black != 0.0f)
        {
            ...
            if (pContent != nullptr)
            {
                pContent->cge += QStringLiteral(" %1(%2, %3, %4, %5)")
                                     .arg(m_argNames[i])
                                     .arg(int(cyan * 100.0f))
                                     .arg(int(magenta * 100.0f))
                                     .arg(int(yellow * 100.0f))
                                     .arg(int(black * 100.0f));
...
{
    preview();
    if (m_filter == nullptr || m_filter->lookupTexture() == 0) return;
...
BasicAdjustWindow::BasicAdjustWindow(MainWindow* mainWindow, QWidget* parent) :
    MenuDialogCommon(parent), m_mainWindow(mainWindow), m_activeAdjustWidget(nullptr)
{
    m_ui.setupUi(this);
