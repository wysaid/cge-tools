...
CurveAdjustWindow::CurveAdjustWindow(MainWindow* mainWindow, QWidget* parent) : MenuDialogCommon(parent), m_mainWindow(mainWindow), m_filter(nullptr)
{
	m_ui.setupUi(this);
...
{	
	if(m_filter == nullptr)
	{
        m_filter = CGE::g_isFastFilterImpossible ? nullptr : static_cast<decltype(m_filter)>(CGE::createMoreCurveFilter());
        if(m_filter == nullptr)
            m_filter = static_cast<decltype(m_filter)>(CGE::createMoreCurveTexFilter());
...
void CurveAdjustWindow::updateFilter()
{
	...
	m_filter->pushCurves(dataRGB.data(), dataRGB.size(), dataRGB.data(), dataRGB.size(), dataRGB.data(), dataRGB.size());
...
void CurveAdjustWindow::apply()
{
	...
	{
		QMessageBox::information(m_mainWindow, QStringLiteral("Invalid Parameters"), QStringLiteral("Curve parameters are invalid or have no effect. Please ensure at least one curve is meaningful."));
		return ;
	}	
}
