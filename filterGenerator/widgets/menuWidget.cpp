...
{
    if (m_menuWidget) m_menuWidget->runProc(m_itemIndex);
}

MenuWidget::MenuWidget(MainWindow* mainWindow, QWidget* parent) :
    QWidget(parent), m_mainWindow(mainWindow)  //, m_popupWidget(nullptr)
{
...
void MenuWidget::runProc(MenuItems index)
{
    ...
    switch (index)
    {
        ...
        case Face_Effect_Tool: {
            m_mainWindow->hide();
            auto w = new FaceEffectToolWindow([&]() { m_mainWindow->show(); }, m_mainWindow);
            w->show();
        }
        break;
        default:
