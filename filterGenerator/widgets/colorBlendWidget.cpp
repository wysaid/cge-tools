...
ColorBlendWindow::ColorBlendWindow(MainWindow* mainWindow, QWidget* parent) :
    MenuDialogCommon(parent),
    ...
    m_texBlendMode(CGE_BLEND_MIX),
    m_colorBlendMode(ColorBlend_Pixel),
    m_vigStart(0.0f),
    m_vigRange(1.0f),
    m_bVigBlendAlpha(true)
{
    m_ui.setupUi(this);
...
bool ColorBlendWindow::checkFilter()
{
    ...
    if (m_wrapFilter == nullptr || m_filter == nullptr || colorBlendMode != m_colorBlendMode ||
        texBlendMode != m_texBlendMode ||
...
bool ColorBlendWindow::_preview()
{
    ...
    {
        case ColorBlend_Pixel:
            if (!_previewBlendPixel()) return false;
            break;
        case ColorBlend_Vignette:
            if (!_previewBlendVignette()) return false;
            break;
        case ColorBlend_Self:
            if (!_previewBlendSelf()) return false;
            break;
        default:
            break;
    }
...
{
    if (m_ui.redEdit->text().isEmpty() || m_ui.greenEdit->text().isEmpty() || m_ui.blueEdit->text().isEmpty() ||
        m_ui.alphaEdit->text().isEmpty())
    {
...
bool ColorBlendWindow::_applyBlendVignette()
{
    ...
    {
        QMessageBox::critical(this, "'start' and 'range' cannot be empty!",
                              "Please fill in the values for 'start' and 'range'!", QMessageBox::Ok);
        return false;
    }
    ...
    if (m_ui.vigStartEdit->text().toFloat() == 0.0f && m_ui.vigRangeEdit->text().toFloat() == 0.0f)
    {
        if (QMessageBox::warning(
                this, "Refusing stupid effect!",
                "Warning: If you insist on setting both 'start' and 'range' to 0, please use the standalone color blend in the first option instead. Are you sure you want to proceed with this?",
    ...
    FormatContent content;
    content.cge =
        blendFormatString.cge.arg(blendMode).arg(r).arg(g).arg(b).arg(a).arg(intensity).arg(m_vigStart).arg(m_vigRange);
    content.android = blendFormatString.android.arg(blendMode)
                          .arg(r)
                          .arg(g)
                          .arg(b)
                          .arg(a)
                          ...
                          .arg(m_vigStart)
                          .arg(m_vigRange);
    content.flash = blendFormatString.flash.arg(blendMode)
                        .arg(r)
                        .arg(g)
                        .arg(b)
                        .arg(a)
...
void ColorBlendWindow::apply()
{
    ...
    {
        case ColorBlend_Pixel:
            if (!_applyBlendPixel()) return;
            break;
        case ColorBlend_Vignette:
            if (!_applyBlendVignette()) return;
            break;
        case ColorBlend_Self:
            if (!_applyBlendSelf()) return;
            break;
        default:
            break;
    }
