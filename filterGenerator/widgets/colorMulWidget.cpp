...
ColorMulWindow::ColorMulWindow(MainWindow* mainWindow, QWidget* parent) :
    MenuDialogCommon(parent), m_mainWindow(mainWindow), m_filter(nullptr), m_mode(CGE::CGEColorMulFilter::mulFLT)
{
    m_ui.setupUi(this);
...
bool ColorMulWindow::checkFilter()
{
    ...
    if (m_ui.matMulRadioBtn->isChecked())
        ...
        mode = CGE::CGEColorMulFilter::mulFLT;

    if (mode == CGE::CGEColorMulFilter::mulFLT) return false;

    if (m_filter == nullptr || m_mode != mode)
    {
        m_mode = mode;
        if (m_filter) m_mainWindow->removeFilter(m_filter);
...
bool ColorMulWindow::_previewVec(FormatContent* pContent)
{
    ...
    {
        pContent->cge =
            QStringLiteral("colormul vec %1 %2 %3").arg(m_vecValues[0]).arg(m_vecValues[1]).arg(m_vecValues[2]);
...
{
    QLineEdit* edits[9] = {m_ui.mat0Edit, m_ui.mat1Edit, m_ui.mat2Edit, m_ui.mat3Edit, m_ui.mat4Edit,
                           m_ui.mat5Edit, m_ui.mat6Edit, m_ui.mat7Edit, m_ui.mat8Edit};

    if (pContent) pContent->cge = "colormul mat";
    ...
    for (int i = 0; i != 9; ++i)
    {
        ...
        if (pContent) pContent->cge += QStringLiteral(" %1").arg(m_matValues[i]);

        if (!edits[i]->text().isEmpty()) isOK = true;
    }
...
void ColorMulWindow::apply()
{
    ...
    if (!_preview(&content))
    {
        QMessageBox::critical(this, "Invalid Param!",
                              "No category selected or parameter error. Please check and re-enter!");
        return;
    }
