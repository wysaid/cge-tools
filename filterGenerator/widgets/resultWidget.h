...
struct FilterResultFormat
{
    ...
    FilterResultFormat(const FormatContent& content, CGE::CGEImageFilterInterfaceAbstract* f) :
        formatContent(content), filter(f)
    {}
...
class ResultWidget : public QWidget
{
    Q_OBJECT
public:
    ResultWidget(MainWindow* mainWindow, QWidget* parent = nullptr) :
        QWidget(parent), m_mainWindow(mainWindow), m_isHover(false)
    {
        m_ui.setupUi(this);
    ...
    static inline bool sortComp(const ResultWidget* left, const ResultWidget* right) { return left->y() < right->y(); }
