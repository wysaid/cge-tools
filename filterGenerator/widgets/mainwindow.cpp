...
void MenuDialogCommon::applyAndQuit()
{
	...
	QMessageBox::information(this, QStringLiteral("Effect Added"), QStringLiteral("The effect was added successfully."), QMessageBox::Ok);
	deleteLater();
}
...
MainWindow::MainWindow(QWidget *parent)
	: QMainWindow(parent), m_stopRunFilters(false), m_shouldSave(false)
{
	m_ui.setupUi(this);
	_initWidgets();
...
void MainWindow::closeEvent(QCloseEvent *e)
{
	...
	{
		if(QMessageBox::question(this, QStringLiteral("Unsaved Work"), QStringLiteral("You have unsaved changes. Exit anyway?"), QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
		{
			e->ignore();
			return ;
		}
...
void MainWindow::openImage()
{
	...
	{
		QMessageBox::warning(this, QStringLiteral("Unexpected Error"), QStringLiteral("Unable to write config file: %1").arg(filename));
	}
}
...
void MainWindow::saveResults()
{
	...
	QString filename = QFileDialog::getSaveFileName(this, QStringLiteral("Save The Result"), QDir::currentPath(), "Config (*.cge);;All files (*)");
	if(filename.size() == 0)
	{
		return ;
	}
...
void MainWindow::_saveImage(const QString& filename)
{
	...
	{
		if(QMessageBox::question(this, QStringLiteral("File Conflict"), QStringLiteral("%1 already exists. Overwrite it?").arg(name), QMessageBox::Yes, QMessageBox::No) == QMessageBox::No)
			return ;
	}
...
void MainWindow::appendStep(CGE::CGEImageFilterInterfaceAbstract* filter, const FormatContent& content)
{
	...
	m_ui.outputWidget->setFixedHeight(CGE::CGE_MAX((MARGIN_TOP + w->height()) * (int)m_resultSteps.size(), m_ui.outputScrollWidget->height() - 20));
