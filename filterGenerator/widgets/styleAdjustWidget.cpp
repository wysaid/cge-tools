...
{
    if (m_ui.motionEdit->text().size() == 0 || m_ui.angleEdit->text().size() == 0 ||
...
{
    QString&& filename =
        QFileDialog::getExistingDirectory(this, QStringLiteral("Choose A Folder!"), m_ui.folderEdit->text());
    if (!filename.isEmpty())
    {
    ...
    if (str != m_filename || texID == 0)
    {
        ...
        if (img.isNull())
        {
            QMessageBox::information(this, "Image Not Found",
                                     "Image does not exist! Please check whether the directory is set correctly!");
            return;
        }
...
{
    if (m_ui.filenameEdit->text().size() == 0 || m_ui.widthEdit->text().size() == 0 ||
        m_ui.heightEdit->text().size() == 0 || m_ui.unitWidthEdit->text().size() == 0 ||
    ...
    if (m_filename.isEmpty() || m_filter == nullptr || m_filter->getTexture() == 0) return;

    FormatContent content;
    content.cge =
        FilterFormats::colorMapping.cge.arg(m_filename).arg(m_width).arg(m_height).arg(m_unitWidth).arg(m_unitHeight);
...
StyleAdjustWindow::StyleAdjustWindow(MainWindow* mainwindow, QWidget* parent) :
