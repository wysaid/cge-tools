...
TextureBlendWindow::TextureBlendWindow(MainWindow* mainWindow, QWidget* parent) :
    ...
    if (m_texture == 0 || imageName != m_texFileName)
    {
        ...
        if (!loadTexture(m_texFileName))
        {
            QMessageBox::information(
                this, QStringLiteral("Invalid Image Name!"),
                "Failed to load texture resource. Please check whether your resource name is filled in correctly!");
            return false;
        }
    }

    if (m_filter == nullptr || m_blendMode != m_ui.blendModeBox->currentIndex() ||
        m_mapFunc != m_ui.mapModeBox->currentIndex())
    {
...
{
    if (_preview()) m_mainWindow->runFilters();
}
...
bool TextureBlendWindow::resetFilter(CGETextureBlendMode mode, MapFunction func)
{
    ...
    {
        if (m_filter) m_filter->setSamplerID(0, false);
