...
#define FILTER_COMMON_ERROR_MSG                                                                        \
    do                                                                                                 \
    {                                                                                                  \
        QMessageBox::information(this, "Invalid Parameters",                                           \
                                 "Some parameter values are invalid or not filled in! Please check."); \
    } while (0)


// Only provides fixed-parameter methods. For variable-parameter methods such as curves or selective colors, please
// modify the corresponding code directly.
struct FilterFormats
{
    typedef struct Format
    {
        Format() {}
        Format(const QString& sCGE, const QString& sAndroid, const QString& sFlash) :
            cge(sCGE), android(sAndroid), flash(sFlash)
        {}
        QString cge;
        QString android;
        QString flash;
...
{
public:
    MenuDialogCommon(QWidget* parent) : QDialog(parent) { setAttribute(Qt::WA_DeleteOnClose); }
    ~MenuDialogCommon() { CGE_LOG_INFO("MenuDialogCommon closed\n"); }
