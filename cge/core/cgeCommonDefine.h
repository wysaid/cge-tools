...
#define CGE_GLES_ATTACH_STRING_L \
    "#version " CGE_GET_MACRO_STRING(_CGE_SHADER_VERSION_) "\n#ifdef GL_ES\nprecision lowp float;\n#endif\n"
#define CGE_GLES_ATTACH_STRING_M \
    "#version " CGE_GET_MACRO_STRING(_CGE_SHADER_VERSION_) "\n#ifdef GL_ES\nprecision mediump float;\n#endif\n"
#define CGE_GLES_ATTACH_STRING_H \
...
template <class T, int Len> static inline int cgeArrLen(const T (&v)[Len])
{
    return Len;
}
...
struct _cgeResetValue
{
    ...
    template <class T> static inline void r(T& t1, T& t2)
    {
        t1 = T();
        t2 = T();
    }
    template <class T> static inline void r(T& t1, T& t2, T& t3)
    {
        t1 = T();
        t2 = T();
        t3 = T();
    }
    template <class T> static inline void r(T& t1, T& t2, T& t3, T& t4)
    {
        t1 = T();
        t2 = T();
        t3 = T();
    ...
    template <class T> static inline void r(T& t1, T& t2, T& t3, T& t4, T& t5)
    {
        t1 = T();
        t2 = T();
        t3 = T();
    ...
    template <class T> static inline void r(T& t1, T& t2, T& t3, T& t4, T& t5, T& t6)
    {
        t1 = T();
        t2 = T();
        t3 = T();
    ...
    template <class T> static inline void r(T& t1, T& t2, T& t3, T& t4, T& t5, T& t6, T& t7)
    {
        t1 = T();
        t2 = T();
        t3 = T();
    ...
    template <class T> static inline void r(T& t1, T& t2, T& t3, T& t4, T& t5, T& t6, T& t7, T& t8)
    {
        t1 = T();
        t2 = T();
        t3 = T();
...
template <typename T> static inline void cgeResetValue(T& t)
{
    t = T();
}

template <typename T, typename... ARGS> static inline void cgeResetValue(T& t, ARGS&... args)
{
    t = T();
    cgeResetValue(args...);
}
...
template <class T> class CGEBlockLimit
{
...
template <class T> inline CGEBlockLimit<const T&> ___cgeMakeBlockLimit(const T& f)
{
...
template <typename Type> inline Type CGE_MIN(Type a, Type b)
{
    return a < b ? a : b;
}
...
template <typename Type> inline Type CGE_MAX(Type a, Type b)
{
    return a > b ? a : b;
}
...
template <typename Type> inline Type CGE_MID(Type n, Type vMin, Type vMax)
{
    if (n < vMin)
        n = vMin;
    else if (n > vMax)
...
extern "C"
{
    ...
    GLuint cgeGenTextureWithBuffer(const void* bufferData, GLint w, GLint h, GLenum channelFmt, GLenum dataFmt,
                                   GLint channels = 4, GLint bindID = 0, GLenum texFilter = GL_LINEAR,
                                   GLenum texWrap = GL_CLAMP_TO_EDGE);
#else
const char* cgeGetBlendModeName(const CGETextureBlendMode mode, bool withChinese);
GLuint cgeGenTextureWithBuffer(const void* bufferData, GLint w, GLint h, GLenum channelFmt, GLenum dataFmt,
                               GLint channels, GLint bindID, GLenum texFilter, GLenum texWrap);
#endif
