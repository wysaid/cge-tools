...
CGEConstString g_vshDefault =
    CGE_SHADER_STRING(attribute vec2 vPosition; attribute vec2 aTexCoord; varying vec2 textureCoordinate; void main() {
        gl_Position = vec4(vPosition, 0.0, 1.0);
        textureCoordinate = aTexCoord;
    });

CGEConstString g_vshDefaultWithoutTexCoord =
    CGE_SHADER_STRING(attribute vec2 vPosition; varying vec2 textureCoordinate; void main() {
        gl_Position = vec4(vPosition, 0.0, 1.0);
        // An opportunism code. Do not use it unless you know what it means.
        textureCoordinate = (vPosition.xy + 1.0) / 2.0;
    });

CGEConstString g_vshDrawToScreen =
    CGE_SHADER_STRING(attribute vec2 vPosition; varying vec2 textureCoordinate; void main() {
        gl_Position = vec4(vPosition, 0.0, 1.0);
...
CGEConstString g_vshDrawToScreenRot90 = CGE_SHADER_STRING(
    attribute vec2 vPosition; varying vec2 textureCoordinate; const mat2 mRot = mat2(0.0, 1.0, -1.0, 0.0); void main() {
        gl_Position = vec4(vPosition, 0.0, 1.0);
        textureCoordinate = vec2(vPosition.x, -vPosition.y) / 2.0 * mRot + 0.5;
    });

CGEConstString g_fshDefault =
...
static CGEConstString s_fshFastAdjust = CGE_SHADER_STRING_PRECISION_M(
    ...
    void main() {
        ...
        gl_FragColor = vec4(curveArray[int(src.r * curvePrecision)].r, curveArray[int(src.g * curvePrecision)].g,
...
static CGEConstString s_fshFastAdjustRGB = CGE_SHADER_STRING_PRECISION_M(
    ...
    void main() {
        vec4 src = texture2D(inputImageTexture, textureCoordinate);
        gl_FragColor = vec4(curveArrayRGB[int(src.r * curvePrecision)], curveArrayRGB[int(src.g * curvePrecision)],
                            curveArrayRGB[int(src.b * curvePrecision)], src.a);
    });
...
// Preprocess CGE_CURVE_PRECISION at program startup
static bool _dummy = s_genFastAdjustShader(s_fshFastAdjustBuffer, s_fshFastAdjust, CGE_CURVE_PRECISION) &&
    s_genFastAdjustShader(s_fshFastAdjustRGBBuffer, s_fshFastAdjustRGB, CGE_CURVE_PRECISION);
...
CGEConstString g_fshCurveMapNoIntensity = CGE_SHADER_STRING_PRECISION_M(
    ...
    void main() {
        vec4 src = texture2D(inputImageTexture, textureCoordinate);
        gl_FragColor = vec4(texture2D(curveTexture, vec2(src.r, 0.0)).r, texture2D(curveTexture, vec2(src.g, 0.5)).g,
                            texture2D(curveTexture, vec2(src.b, 1.0)).b, src.a);
    });
...
CGEImageFilterInterfaceAbstract::CGEImageFilterInterfaceAbstract()
{
    ...
    CGE_UNEXPECTED_ERR_MSG(CGE_LOG_KEEP("Filter create, total: %d\n", ++sFilterCount);)
}

CGEImageFilterInterfaceAbstract::~CGEImageFilterInterfaceAbstract()
{
    CGE_UNEXPECTED_ERR_MSG(CGE_LOG_KEEP("Filter release, remain: %d\n", --sFilterCount);)
}
...
void CGEImageFilterInterface::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
