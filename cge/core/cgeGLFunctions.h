...
// The return value of CGEBufferLoadFun will be passed as the first argument to CGEBufferUnloadFun
// The arg parameter of CGEBufferLoadFun will be passed as the second argument to CGEBufferUnloadFun
typedef void* (*CGEBufferLoadFun)(const char* sourceName, void** bufferData, GLint* w, GLint* h, CGEBufferFormat* fmt,
                                  void* arg);
typedef bool (*CGEBufferUnloadFun)(void* arg1, void* arg2);

// Texture load callback. Note: to keep the interface simple, the texture unit returned by the callback must be released
// by the caller. The returned texture must not be a special texture type that glDeleteTextures cannot handle.
...
public:
    ...
    SharedTexture(const SharedTexture& other) : m_textureID(0), m_refCount(nullptr) { *this = other; }
    ...
    // Special usage: must be paired with forceRelease
    inline void forceAssignTextureID(GLuint texID) { m_textureID = texID; }
...
public:
    ...
    inline void bindTexture2D(const SharedTexture& texture, GLsizei x, GLsizei y, GLsizei w, GLsizei h,
    ...
    inline void bindTexture2D(GLuint texID, GLenum attachment = GL_COLOR_ATTACHMENT0) const
    {
        ...
        CGE_LOG_CODE(GLenum code = glCheckFramebufferStatus(GL_FRAMEBUFFER); if (code != GL_FRAMEBUFFER_COMPLETE) {
            CGE_LOG_ERROR("CGE::FrameBuffer::bindTexture2D - Frame buffer is not valid: %x\n", code);
        })
    }

    inline void bindTexture2D(GLuint texID, GLsizei x, GLsizei y, GLsizei w, GLsizei h,
                              GLenum attachment = GL_COLOR_ATTACHMENT0) const
    {
...
struct CGESizei
{
    ...
    bool operator==(const CGESizei& other) const { return width == other.width && height == other.height; }
    bool operator!=(const CGESizei& other) const { return width != other.width || height != other.height; }
    GLint width;
    GLint height;
};
...
struct CGELuminance
{
    ...
    static inline int RGB888(int r, int g, int b) { return (r * RWeight + g * GWeight + b * BWeight) >> CalcPrecision; }

    // Color byte order from low to high is r-g-b; be aware of endianness when passing parameters
