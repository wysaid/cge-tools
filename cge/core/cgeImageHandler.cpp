...
CGE_UNEXPECTED_ERR_MSG(static int sHandlerCount = 0;)
...
CGEImageHandlerInterface::CGEImageHandlerInterface() : m_srcTexture(0), m_dstFrameBuffer(0), m_vertexArrayBuffer(0)
{
    ...
    CGE_UNEXPECTED_ERR_MSG(CGE_LOG_KEEP("Handler create, total: %d\n", ++sHandlerCount);)
}
...
CGEImageHandlerInterface::~CGEImageHandlerInterface()
{
    ...
    CGE_UNEXPECTED_ERR_MSG(CGE_LOG_KEEP("Handler release, remain: %d\n", --sHandlerCount);)
}
...
void CGEImageHandlerInterface::copyTextureData(void* data, int w, int h, GLuint texID, GLenum dataFmt,
                                               GLenum channelFmt)
{
    ...
    glFinish();
    if (channelFmt != GL_RGBA) glPixelStorei(GL_PACK_ALIGNMENT, 1);
...
bool CGEImageHandlerInterface::initImageFBO(const void* data, int w, int h, GLenum channelFmt, GLenum dataFmt,
                                            int channel)
{
    clearImageFBO();

    if (m_vertexArrayBuffer == 0) m_vertexArrayBuffer = cgeGenCommonQuadArrayBuffer();
...
bool CGEImageHandler::initWithRawBufferData(const void* imgData, GLint w, GLint h, CGEBufferFormat format,
...
bool CGEImageHandler::updateData(const void* data, int w, int h, CGEBufferFormat format)
{
    ...
    GLenum dataFmt, channelFmt;
    cgeGetDataAndChannelByFormat(format, &dataFmt, &channelFmt, &channel);
    if (!(w == m_dstImageSize.width && h == m_dstImageSize.height && channel == 4)) return false;
...
{
    if (textureID == 0 || w < 1 || h < 1) return false;
    ...
    if (!m_bRevertEnabled) m_srcTexture = 0;
    return true;
}
...
{
    if (!g_shouldUsePBO || m_pixelPackBuffer == 0) return nullptr;
...
bool CGEImageHandler::getOutputBufferData(void* data, CGEBufferFormat fmt)
{
    ...
    {
        CGE_LOG_ERROR("%s\n",
                      nullptr == data ? "data is NULL" :
                                        (channel == 4 ? "Handler is not initialized!" : "Channel must be 4!"));
        return false;
    }
    CGE_ENABLE_GLOBAL_GLCONTEXT();
    setAsTarget();
    ...
    if (g_shouldUsePBO)
    {
        ...
        if (m_pixelPackBuffer != 0)
        {
            ...
            glReadPixels(0, 0, m_dstImageSize.width, m_dstImageSize.height, channelFmt, dataFmt, 0);
            GLubyte* bytes =
                (GLubyte*)glMapBufferRange(GL_PIXEL_PACK_BUFFER, 0, m_pixelPackBufferSize, GL_MAP_READ_BIT);
...
{
    if (m_bufferTextures[0] == 0 || m_dstFrameBuffer == 0) return 0;
    return m_dstImageSize.width * m_dstImageSize.height * channel;
}
...
void CGEImageHandler::swapBufferFBO()
{
    ...
    CGE_LOG_CODE(
        ...
        } else { CGE_LOG_INFO("Swapping buffer FBO...\n"); })
}
...
bool CGEImageHandler::copyTexture(GLuint dst, GLuint src)
{
    ...
    if (hasBlending) glDisable(GL_BLEND);
    ...
    if (hasBlending) glEnable(GL_BLEND);
...
{
    if (m_resultDrawer == nullptr) m_resultDrawer = TextureDrawer::create();
    return m_resultDrawer;
}

void CGEImageHandler::setResultDrawer(TextureDrawer* drawer)
{
    if (m_resultDrawer != nullptr) delete m_resultDrawer;
    m_resultDrawer = drawer;
}

GLuint CGEImageHandler::copyLastResultTexture(GLuint texID)
{
    if (m_bufferTextures[1] == 0 || m_dstFrameBuffer == 0) return texID;
    CGE_ENABLE_GLOBAL_GLCONTEXT();

    if (texID == 0)
        texID =
            cgeGenTextureWithBuffer(nullptr, m_dstImageSize.width, m_dstImageSize.height, GL_RGBA, GL_UNSIGNED_BYTE);
...
void CGEImageHandler::processingFilters()
{
    ...
    for (std::vector<CGEImageFilterInterfaceAbstract*>::iterator iter = m_vecFilters.begin(); iter < m_vecFilters.end();
         ++iter)
    {
        swapBufferFBO();
...
{
    if (index == -1) index = (GLint)m_vecFilters.size() - 1;
...
{
    if (!m_bRevertEnabled || m_bufferTextures[0] == 0 || m_dstFrameBuffer == 0) return false;
...
bool CGEImageHandler::revertToKeptResult(bool bRevert2Target)
{
    ...
    if (m_drawer == nullptr)
    {
        ...
        if (bRevert2Target)
        {
            ...
            // glCopyTexSubImage2D will block the CPU
            glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, m_dstImageSize.width, m_dstImageSize.height);
            glFlush();
        }
        glBindTexture(GL_TEXTURE_2D, m_bufferTextures[0]);
...
int CGEImageHandler::getFilterIndexByAddr(const void* addr)
{
    ...
    for (int i = 0; i != sz; ++i)
    {
        if (addr == m_vecFilters[i]) return i;
    }
    return -1;
}
...
{
    if (m_vecFilters.empty()) return false;
...
{
    if (left == right || left >= m_vecFilters.size() || right >= m_vecFilters.size()) return false;
