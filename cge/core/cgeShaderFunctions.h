...
public:
    ...
    {
        UniformData() {}
        UniformData(const char* name, UNIFORM_TYPE type) : uniformType(type)
        {
            strncpy(uniformName, name, CGE_UNIFORM_MAX_LEN);
        }
...
protected:
    inline GLint _getUniform(GLuint programId, const char* name)
    {
        ...
        CGE_LOG_CODE(if (uniform < 0) CGE_LOG_ERROR("uniform name %s does not exist!\n", name););
        return uniform;
    }
