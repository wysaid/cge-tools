...
extern "C"
{
    ...
    bool _cgeCheckGLError(const char* name, const char* file, int line)
    {
        ...
        {
            CGE_LOG_CODE(const char* pMsg; switch (error) {
    ...
    {
        if (mode < 0 || mode >= CGE_BLEND_TYPE_MAX_NUM) return nullptr;
    ...
    GLuint cgeGenTextureWithBuffer(const void* bufferData, GLint w, GLint h, GLenum channelFmt, GLenum dataFmt,
                                   GLint channel, GLint bindID, GLenum texFilter, GLenum texWrap)
    {
        assert(w != 0 && h != 0);
        GLuint tex;
