...
CGE_LOG_CODE(static int sTextureCount;)
...
{
    if (s_enableGLFunc) s_enableGLFunc(s_enableGLParam);
}

void cgeDisableGlobalGLContext()
{
    if (s_disableGLFunc) s_disableGLFunc(s_disableGLParam);
}
...
{
    if (s_unloadBufferDataCommon != nullptr) return s_unloadBufferDataCommon(bufferArg, s_cgeUnloadParam);
    return false;
}
...
SharedTexture::SharedTexture(GLuint textureID, int w, int h)
{
    ...
    width = w;
    height = h;
    CGE_LOG_CODE(if (m_textureID == 0) CGE_LOG_ERROR("CGESharedTexture : Invalid TextureID!"); else {
...
SharedTexture::~SharedTexture()
{
    ...
    {
        CGE_LOG_CODE(if (m_textureID != 0) { CGE_LOG_ERROR("SharedTexture : Error occurred!"); });
        return;
    }
    ...
    CGE_LOG_CODE(else { CGE_LOG_INFO("@@@ Texture %d deRef count: %d\n", m_textureID, *m_refCount); })
}
...
void SharedTexture::forceRelease(bool bDelTexture)
{
    ...
    assert(m_refCount == nullptr ||
           *m_refCount == 1);  // When using forceRelease, SharedTexture must guarantee only one instance exists

    if (bDelTexture) glDeleteTextures(1, &m_textureID);
    m_textureID = 0;
    CGE_DELETE(m_refCount);
    width = 0;
    height = 0;
    CGE_LOG_CODE(--sTextureCount;);
}

void SharedTexture::clear()
{
    CGE_LOG_CODE(
        if (m_textureID == 0) { CGE_LOG_ERROR("!!!CGESharedTexture - Invalid TextureID To Release!\n"); } else {
            CGE_LOG_INFO("###CGESharedTexture deleting, textureID %d, now total : %d ###\n", m_textureID,
                         --sTextureCount);
        });

    assert(*m_refCount == 0);  // Unknown error
