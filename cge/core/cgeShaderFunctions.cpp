...
CGE_UNEXPECTED_ERR_MSG(static int sProgramCount = 0;)
...
bool ShaderObject::init(GLenum shaderType)
{
    ...
    if (m_shaderID == 0) m_shaderID = glCreateShader(m_shaderType);
    return m_shaderID != 0;
}
...
bool ShaderObject::loadShaderSourceFromString(const char* shaderString)
{
    ...
    CGE_LOG_CODE(GLint logLen; glGetShaderiv(m_shaderID, GL_INFO_LOG_LENGTH, &logLen); if (logLen > 0) {
...
{
    for (std::vector<UniformData*>::iterator iter = m_vecUniforms.begin(); iter != m_vecUniforms.end(); ++iter)
        delete *iter;
    m_vecUniforms.clear();
}
    ...
    for (std::vector<UniformData*>::iterator iter = m_vecUniforms.begin(); iter != m_vecUniforms.end(); ++iter)
    {
        ...
        switch ((*iter)->uniformType)
        {
            ...
            case uniformINTV2:
                ...
                break;
            case uniformINTV3:
                glUniform3i(uniformID, (*iter)->uniformValue[0].valuei, (*iter)->uniformValue[1].valuei,
                            (*iter)->uniformValue[3].valuei);
                break;
            case uniformINTV4:
                glUniform4i(uniformID, (*iter)->uniformValue[0].valuei, (*iter)->uniformValue[1].valuei,
                            (*iter)->uniformValue[2].valuei, (*iter)->uniformValue[3].valuei);
                break;
            case uniformFLOAT:
            ...
            case uniformFLOATV2:
                ...
                break;
            case uniformFLOATV3:
                glUniform3f(uniformID, (*iter)->uniformValue[0].valuef, (*iter)->uniformValue[1].valuef,
                            (*iter)->uniformValue[2].valuef);
                break;
            case uniformFLOATV4:
                glUniform4f(uniformID, (*iter)->uniformValue[0].valuef, (*iter)->uniformValue[1].valuef,
                            (*iter)->uniformValue[2].valuef, (*iter)->uniformValue[3].valuef);
                break;
...
ProgramObject::ProgramObject()
{
    ...
    CGE_UNEXPECTED_ERR_MSG(CGE_LOG_KEEP("ProgramObject create, total: %d\n", ++sProgramCount);)
}

ProgramObject::~ProgramObject()
{
    CGE_UNEXPECTED_ERR_MSG(CGE_LOG_KEEP("ProgramObject release, remain: %d\n", --sProgramCount);)
