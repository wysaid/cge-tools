...
public:
    ...
    virtual void rotate(float dRot) { m_rotation += dRot; }

    virtual void rotateTo(float rot) { m_rotation = rot; }
    ...
    inline bool operator<(const SpriteInterface2d& other) const { return m_zIndex < other.m_zIndex; }
...
public:
    ...
    // The Cartesian coordinate system has an inverted Y-axis compared to screen coordinates, so the default is
    // vertically flipped. To draw the sprite into a texture, disable the flip.
...
public:
    ...
    void pushZonePoint(float x, float y)
    {
        SharedPoint pnt(x, y);
        m_zone.push_back(pnt);
    }
...
public:
    Sprite2dInterChangeExt(GLuint textureID, int width, int height) :
        Sprite2dInterChange(textureID, width, height),
        m_frameIndex(0),
        m_deltaTime(100.0f),
        m_deltaAccum(0.0f),
        m_lastTime(0.0f),
        ...
        m_shouldLoop(true)
    {}
    Sprite2dInterChangeExt(const SharedTexture& texture) :
        Sprite2dInterChange(texture),
        m_frameIndex(0),
        m_deltaTime(100.0f),
        m_deltaAccum(0.0f),
        m_lastTime(0.0f),
...
public:
    Sprite2dSequence(int width, int height) :
        Sprite2d(-1, width, height),
        m_frameIndex(0),
        m_deltaTime(100.0),
        m_deltaAccum(0.0),
        m_lastTime(0.0),
...
public:
    ...
    void rotate(float rot, float x, float y, float z) { m_rotMatrix.rotate(rot, x, y, z); }

    void rotate(float rot, const Vec3f& axis) { m_rotMatrix.rotate(rot, axis[0], axis[1], axis[2]); }

    void rotateX(float rot) { m_rotMatrix.rotateX(rot); }

    void rotateY(float rot) { m_rotMatrix.rotateY(rot); }

    void rotateZ(float rot) { m_rotMatrix.rotateZ(rot); }

    void rotateTo(float rot, float x, float y, float z) { m_rotMatrix = Mat3::makeRotation(rot, x, y, z); }

    void rotateTo(float rot, const Vec3f& axis) { m_rotMatrix = Mat3::makeRotation(rot, axis[0], axis[1], axis[2]); }

    void rotateXTo(float rot) { m_rotMatrix = Mat3::makeXRotation(rot); }

    void rotateYTo(float rot) { m_rotMatrix = Mat3::makeYRotation(rot); }

    void rotateZTo(float rot) { m_rotMatrix = Mat3::makeZRotation(rot); }
