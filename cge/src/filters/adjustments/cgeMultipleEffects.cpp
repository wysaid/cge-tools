...
#include "../internal/cgeMultipleEffectsDataset.h"
#include "cgeDataParsingEngine.h"
#include <cmath>
#include <cctype>

static CGEConstString s_fshHTFilterMix =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D originImageTexture; uniform float intensity;
...
CGEMutipleEffectFilter::CGEMutipleEffectFilter() :
    m_loadFunc(nullptr),
    m_unloadFunc(nullptr),
    m_loadParam(nullptr),
    m_unloadParam(nullptr),
    m_texCache(0),
    m_isWrapper(false),
...
{
    for (std::vector<CGEImageFilterInterface*>::iterator iter = m_vecFilters.begin(); iter != m_vecFilters.end();
         ++iter)
    {
        delete *iter;
    }
...
void CGEMutipleEffectFilter::setBufferLoadFunction(CGEBufferLoadFun fLoad, void* loadParam, CGEBufferUnloadFun fUnload,
                                                   void* unloadParam)
{
    m_loadFunc = fLoad;
...
GLuint CGEMutipleEffectFilter::loadResources(const char* textureName, int* width, int* height)
{
    ...
    // Prefer texture loader if available.
    ...
    // No tex loader or tex loader failed; try buffer loader
...
bool CGEMutipleEffectFilter::initWithEffectString(const char* effectString)
{
    ...
    {
        CGE_LOG_ERROR("Effect string \"%s\" failed to generate any effect!\n", effectString);
        return false;
    }
...
void CGEMutipleEffectFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    ...
    {
        CGE_LOG_CODE(
            if (m_vecFilters.empty()) { CGE_LOG_ERROR("CGEMutipleEffectFilter::render2Texture did nothing!\n"); }
    ...
    if (needMix)
    {
        if (m_texCache == 0 || sz != m_currentSize)
        {
            m_currentSize = sz;
            glDeleteTextures(1, &m_texCache);
            m_texCache = cgeGenTextureWithBuffer(nullptr, m_currentSize.width, m_currentSize.height, GL_RGBA,
                                                 GL_UNSIGNED_BYTE, 4, 0, GL_NEAREST, GL_CLAMP_TO_EDGE);
        }
