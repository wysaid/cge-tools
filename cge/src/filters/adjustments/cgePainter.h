...
struct PathAttrib
{
    PathAttrib() :
        ...
        pointStride(3.0f),
        kind(PATH_NORMAL),
        brushStyle(BRUSH_DEFAULT) /*, rotation(0.0f) */
    {}
    ~PathAttrib() {}

    void pushPoint(Vec2f point) { vecPoints.push_back(point); }

    void pushPoint(GLfloat x, GLfloat y) { vecPoints.push_back(Vec2f(x, y)); }
    ...
    /*
    Note:
      The final brightness formula is approximately: opacity * (pointSize / pointStride)
      This means: the larger pointSize is and the smaller pointStride is, the more opaque the path will be.
      To achieve the desired effect, reduce opacity proportionally, or scale pointSize and pointStride by the same
    ratio.
    */
    ...
    enum BrushStyle
    {
        ...
        BRUSH_TEST5,
        MAX_BRUSH_STYLE_NUM
    } brushStyle;  // Keep the brush status.
...
struct GPUPainterTextureManager
{
    ...
    // Called when exiting the current edit session
    static void clearTextures();

    static void setLoadFunction(CGEBufferLoadFun loadFunc, void* loadParam, CGEBufferUnloadFun unloadFunc,
                                void* unloadParam)
    {
        texLoadFunc = loadFunc;
        texLoadParam = loadParam;
        texUnloadFunc = unloadFunc, texUnLoadParam = unloadParam;
    }
...
public:
    ...
    virtual bool resizeCanvas(int width, int height, const void* buffer = nullptr,
                              CGEBufferFormat format = CGE_FORMAT_RGBA_INT8);
...
public:
    ...
    static void genInterpolationPoints(const std::vector<Vec2f>& srcPoints, std::vector<Vec2f>& resultPoints,
                                       Vec2f& canvasSize, GLfloat stride);
    static void genInterpolationPointsAndDirections(const std::vector<Vec2f>& srcPoints,
                                                    std::vector<Vec2f>& resultPoints, std::vector<Vec2f>& dirPoints,
                                                    Vec2f& canvasSize, GLfloat stride);

    static void genInterpolationPointsAndRandomDir(const std::vector<Vec2f>& srcPoints,
...
// Special Painter — incompatible with the blend mode of the painters above; no special settings can be applied.
class GPUPainterMosaicLayer : public GPUPainterLayer
{
public:
    virtual bool init();
    // Mosaic block size, range: >= 1 pixel, 1 means original image
