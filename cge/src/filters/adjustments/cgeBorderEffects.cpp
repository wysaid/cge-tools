...
#include "../internal/cgeBordersDataset.h"
...
bool CGEBorderFilter::initWithBorderString(const char* ptr, bool isShortCut)
{
    ...
    //////////////////////////////////////////////////////////////////////////
    if (isShortCut) return initWithBorderShortcutString(ptr);
...
{
    for (std::vector<CGEBorderFilterInterface*>::iterator iter = m_vecProc.begin(); iter != m_vecProc.end(); ++iter)
    {
...
void CGEBorderFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    if (m_vecProc.empty())
    {
...
void CGEBorderFilter::setLoadFunction(CGEBufferLoadFun fLoad, void* loadParam, CGEBufferUnloadFun fUnload,
                                      void* unloadParam)
{
    m_loadFunc = fLoad;
...
GLuint CGEBorderFilter::loadResourceToTexture(const char* srcName)
{
    ...
    CGEBufferFormat fmt;
    void* pArg;
    if (fLoad == nullptr || (pArg = fLoad(srcName, &bufferData, &w, &h, &fmt, loadParam)) == nullptr)
    {
    ...
    if (fUnload != nullptr) fUnload(pArg, unloadParam);
    return texture;
}
