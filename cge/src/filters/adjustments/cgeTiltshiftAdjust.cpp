...
static CGEConstString s_fshTiltshiftVector = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D blurredImageTexture;
    uniform vec2 blurGradient;  // x for start, y for range.
    uniform vec2 blurPassPos; uniform vec2 blurNormal; uniform vec2 vSteps;
...
static CGEConstString s_fshTiltshiftEllipse = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D blurredImageTexture;
    uniform float blurGradient; uniform vec2 blurCentralPos; uniform vec2 radiusStart; uniform mat2 mRot;
    uniform vec2 vSteps;
...
{
    if (m_samplerScale == radius) return;
...
void CGETiltshiftVectorFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
...
{
    if (m_blurProc.initWithoutFixedRadius() &&
        initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshTiltshiftEllipse))
    {
...
void CGETiltshiftEllipseFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                               GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...
{
    if (m_blurProc.initWithoutFixedRadius(false) &&
        initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshTiltshiftVector))
    {
        setBlurGradient(100.0f, 100.0f);
        setBlurPassPos(0.0f, 0.0f);
        setBlurNormal(1.0f, 1.0f);
