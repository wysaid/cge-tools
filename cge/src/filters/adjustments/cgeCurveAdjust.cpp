...
#include "../../core/cgeCommonDefine.h"
...
namespace CGE
{
void CGECurveInterface::loadCurves(const float* curveR, size_t nR, const float* curveG, size_t nG, const float* curveB,
                                   size_t nB)
{
    if (curveR != nullptr) CGECurveInterface::loadCurve(m_curve, curveR, nR, CHANNEL_R, 1u, 0u);
    if (curveG != nullptr) CGECurveInterface::loadCurve(m_curve, curveG, nG, CHANNEL_G, 1u, 0u);
    if (curveB != nullptr) CGECurveInterface::loadCurve(m_curve, curveB, nB, CHANNEL_B, 1u, 0u);
}
...
bool CGECurveInterface::loadCurve(std::vector<float>& vec, const float* curve, size_t cnt)
{
    ...
    if (vec.size() != cnt) vec.resize(cnt);
    vec.resize(cnt);
    for (int i = 0; i != cnt; ++i)
    {
        vec[i] = curve[i];
...
bool CGECurveInterface::loadCurve(std::vector<CurveData>& vec, const float* curve, size_t cnt, size_t dstChannel,
                                  size_t srcChannel, size_t stride)
{
...
void CGECurveInterface::setPoints(const CurvePoint* r, size_t nr, const CurvePoint* g, size_t ng, const CurvePoint* b,
                                  size_t nb)
{
...
bool CGECurveInterface::mergeCurveConst(std::vector<float>& dst, const std::vector<float>& late,
                                        const std::vector<float>& early)
{
...
bool CGECurveInterface::mergeCurveConst(std::vector<CurveData>& dst, const std::vector<CurveData>& late,
                                        const std::vector<CurveData>& early)
{
    if (early.empty() || early.size() != late.size())
        return false;  // You should call "scaleCurve" to make the sizes equal. (none-zero)
...
bool CGECurveInterface::mergeCurve(std::vector<CurveData>& dst, std::vector<CurveData>& late,
                                   std::vector<CurveData>& early)
{
    if (early.size() != late.size())
    {
        scaleCurve(late, CGE_CURVE_PRECISION);
...
bool CGECurveInterface::mergeCurve(std::vector<CurveData>& dst, std::vector<float>& late, std::vector<CurveData>& early,
                                   unsigned channel)
{
    if (early.empty() || channel >= 3) return false;
...
bool CGECurveInterface::_genCurve(float* curve, const CurvePoint* pnts, size_t cnt, unsigned channel, unsigned stride)
{
    ...
    for (int i = 1; i != cnt - 1; ++i)
    {
        ...
        ypp[i] = (sig - 1.0f) / p;
        u[i] = ((pnts[i + 1].y - pnts[i].y) / (pnts[i + 1].x - pnts[i].x) -
                (pnts[i].y - pnts[i - 1].y) / (pnts[i].x - pnts[i - 1].x));
    ...
    for (int i = 0; i != CGE_CURVE_PRECISION; ++i)
    {
        ...
        const float g =
            a * pnts[kL].y + b * pnts[kH].y + ((a * a * a - a) * ypp[kL] + (b * b * b - b) * ypp[kH]) * (h * h) / 6.0f;
...
bool CGECurveInterface::genCurve(std::vector<CurveData>& vec, const CurvePoint* pntsR, size_t cntR,
                                 const CurvePoint* pntsG, size_t cntG, const CurvePoint* pntsB, size_t cntB)
{
    if (vec.size() != CGE_CURVE_PRECISION) vec.resize(CGE_CURVE_PRECISION);
    if (cntR <= 1 || pntsR == nullptr || cntG <= 1 || pntsG == nullptr || cntB <= 1 || pntsB == nullptr)
    {
        resetCurve(vec, CGE_CURVE_PRECISION);
        CGE_LOG_ERROR("Invalid Curve Points!\nR: %p, Count: %d\nG: %p, Count: %d\nB: %p, Count: %d\n", pntsR, (int)cntR,
                      pntsG, (int)cntG, pntsB, (int)cntB);
        return false;
    }

    return _genCurve(&vec[0][0], pntsR, cntR, 3, 0) && _genCurve(&vec[0][0], pntsG, cntG, 3, 1) &&
...
void CGECurveTexFilter::setPoints(const CurvePoint* r, size_t nr, const CurvePoint* g, size_t ng, const CurvePoint* b,
                                  size_t nb)
{
...
void CGECurveTexFilter::loadCurves(const float* curveR, size_t nR, const float* curveG, size_t nG, const float* curveB,
                                   size_t nB)
{
...
void CGECurveFilter::setPoints(const CurvePoint* r, size_t nr, const CurvePoint* g, size_t ng, const CurvePoint* b,
                               size_t nb)
{
    CGECurveInterface::setPoints(r, nr, g, ng, b, nb);
    flush();
...
void CGECurveFilter::loadCurves(const float* curveR, size_t nR, const float* curveG, size_t nG, const float* curveB,
                                size_t nB)
{
    CGECurveInterface::loadCurves(curveR, nR, curveG, nG, curveB, nB);
    flush();
...
void CGEMoreCurveFilter::pushPoints(const CurvePoint* r, size_t nr, const CurvePoint* g, size_t ng, const CurvePoint* b,
                                    size_t nb)
{
    pushPointsR(r, nr);
    pushPointsG(g, ng);
...
void CGEMoreCurveFilter::pushCurves(const float* curveR, size_t nR, const float* curveG, size_t nG, const float* curveB,
                                    size_t nB)
{
