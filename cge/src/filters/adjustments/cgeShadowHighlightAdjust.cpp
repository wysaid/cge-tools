...
const static char* const s_fshShadowHighlight = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform float shadows;
    ...
    vec3 highlightAndShadow(vec3 src, float l, float d)  // l for highlight, d for shadow
    {
        ...
        // else
        // 	src.r = 4.0 * (l - 1.0) * src.r * src.r * src.r + 8.0 * (1.0 - l) * src.r * src.r + (5.0 * l - 4.0) * src.r
        // + 1.0 - l;

        // if(src.g < 0.5)
        // 	src.g = 4.0 * (d - 1.0) * src.g * src.g * src.g + 4.0 * (1.0 - d) * src.g * src.g + d * src.g;
        // else
        // 	src.g = 4.0 * (l - 1.0) * src.g * src.g * src.g + 8.0 * (1.0 - l) * src.g * src.g + (5.0 * l - 4.0) * src.g
        // + 1.0 - l;

        // if(src.b < 0.5)
        // 	src.b = 4.0 * (d - 1.0) * src.b * src.b * src.b + 4.0 * (1.0 - d) * src.b * src.b + d * src.b;
        // else
        // 	src.b = 4.0 * (l - 1.0) * src.b * src.b * src.b + 8.0 * (1.0 - l) * src.b * src.b + (5.0 * l - 4.0) * src.b
        // + 1.0 - l;
...
void main()
{
    ...
    float shadow = clamp((pow(luminance, 1.0/(shadows+1.0)) + (-0.76)*pow(luminance, 2.0/(shadows+1.0))) - luminance,
0.0, 1.0); float highlight = clamp((1.0 - (pow(1.0-luminance, 1.0/(2.0-highlights)) +
(-0.8)*pow(1.0-luminance, 2.0/(2.0-highlights)))) - luminance, -1.0, 0.0); vec3 result = (luminance + shadow +
highlight) * (source.rgb / (luminance - 0.0));
...
void CGEShadowHighlightFastFilter::setShadowAndHighlight(float shadow, float highlight)
{
    ...
    for (std::vector<float>::size_type t = 0; t != sz; ++t)
    {
        ...
        if (v < 0.5)
            ...
            m_curveRGB[t] =
                4.0f * (l - 1.0f) * v * v * v + 8.0f * (1.0f - l) * v * v + (5.0f * l - 4.0f) * v + 1.0f - l;
    }
    assignCurveArray();
}
