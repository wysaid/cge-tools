...
static CGEConstString s_fshBorderCard = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform vec2 start; uniform vec2 range; uniform vec2 end; uniform float align;

    void main(void) {
        vec2 scale;
        vec2 stepsStart;

        if (textureCoordinate.x >= start.x && textureCoordinate.x <= end.x && textureCoordinate.y >= start.y &&
            textureCoordinate.y <= end.y)
        {
