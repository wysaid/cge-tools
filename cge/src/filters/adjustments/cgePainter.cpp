...
#include "../internal/cgePainterDataset.h"
#include <random>

#define TEXTURE_BRUSH_STYLE CGE_TEXTURE_START
#define TEXTURE_BRUSH_STYLE_INDEX CGE_TEXTURE_START_INDEX

static CGEConstString s_vshPaint =
    CGE_SHADER_STRING(attribute vec4 vPosition; uniform mat4 uMat; uniform float pointSize;
...
static CGEConstString s_fshPaint =
    CGE_SHADER_STRING_PRECISION_M(uniform sampler2D brushTexture; uniform vec4 vColor;

                                  void main() { gl_FragColor = vColor * texture2D(brushTexture, gl_PointCoord); });
...
static CGEConstString s_vshPaintRotWithRad = CGE_SHADER_STRING(
    attribute vec4 vPosition; attribute float pointRadian;

    uniform mat4 uMat; uniform float pointSize; varying mat2 mRot; varying float pointScale;
...
static CGEConstString s_vshPaintRotWithNorm = CGE_SHADER_STRING(
    attribute vec4 vPosition; attribute vec2 vPointNorm;
    ...
    mat2 makeRotation(vec2 norm) {
        ...
        if (len == 0.0) return mat2(1.0, 0.0, 0.0, 1.0);
...
static CGEConstString s_fshPaintRot = CGE_SHADER_STRING_PRECISION_M(
    uniform sampler2D brushTexture; uniform vec4 vColor; varying mat2 mRot; varying float pointScale;

    void main() { gl_FragColor = vColor * texture2D(brushTexture, (gl_PointCoord - 0.5) * pointScale * mRot + 0.5); });
...
static CGEConstString s_fshPainterMosaicMode = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform vec2 samplerSteps; uniform float intensity; uniform float blurPixels;
...
CGEBufferLoadFun GPUPainterTextureManager::texLoadFunc = nullptr;
CGEBufferUnloadFun GPUPainterTextureManager::texUnloadFunc = nullptr;
void *GPUPainterTextureManager::texLoadParam = nullptr, *GPUPainterTextureManager::texUnLoadParam = nullptr;

GLuint GPUPainterTextureManager::getTextureByBrushStyle(PathAttrib::BrushStyle style)
{
    CGE_LOG_CODE(if (style < 0 || style > PathAttrib::MAX_BRUSH_STYLE_NUM) {
...
GLuint GPUPainterTextureManager::loadBrushTexture(PathAttrib::BrushStyle style)
{
    ...
    if (name == nullptr || loadFunc == nullptr) return 0;
    void* buffer = nullptr;
    int w, h;
    CGEBufferFormat fmt;
    void* loadRet = loadFunc(name, &buffer, &w, &h, &fmt, loadParam);
    ...
    if (unloadFunc != nullptr) unloadFunc(loadRet, unloadParam);
    return texID;
}

//////////////////////////////////////////////////////////////////////////

CGEConstString GPUPainterLayer::paramDraw2ScreenTextureName = CGEImageFilterInterface::paramInputImageName;
...
GPUPainterLayer::GPUPainterLayer() :
    m_draw2ScreenProgram(nullptr),
    m_canvasTex(0),
    m_pathIndex(0),
    m_blendMode(CGE_BLEND_MIX),
    m_opacity(1.0f),
    m_blendProgram(nullptr),
...
void GPUPainterLayer::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    handler->setAsTarget();
    usePainterBlendMode();
...
bool GPUPainterLayer::resizeCanvas(int width, int height, const void* buffer, CGEBufferFormat format)
{
    ...
    if (m_canvasSize[0] == width && m_canvasSize[1] == height && buffer == nullptr && m_canvasTex != 0) return true;
    m_canvasSize[0] = (float)width;
    m_canvasSize[1] = (float)height;
    if (m_frameBuffer == 0) glGenFramebuffers(1, &m_frameBuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, m_frameBuffer);
    glDeleteTextures(1, &m_canvasTex);
    GLenum channelFmt, dataFmt;
    cgeGetDataAndChannelByFormat(format, &dataFmt, &channelFmt, nullptr);
...
bool GPUPainterLayer::drawResultToScreen(int x, int y, int w, int h)
{
    ...
    glDisable(GL_BLEND);
    glViewport(x, y, w, h);
    if (m_draw2ScreenProgram == nullptr)
    {
...
{
    if (attrib.vecPoints.empty()) return;
    setPaintEnv();
...
void GPUPainterLayer::redrawPathToIndex()
{
    ...
    for (std::vector<PathAttrib>::iterator iter = m_vecPath.begin(), iEnd = iter + m_pathIndex; iter != iEnd; ++iter)
    {
        drawPath(*iter);
    }
}
...
{
    if (indexStart >= indexEnd || indexStart >= m_vecPath.size()) return;

    if (indexEnd > m_vecPath.size()) indexEnd = m_vecPath.size();

    for (std::vector<PathAttrib>::iterator iter = m_vecPath.begin() + indexStart, iEnd = m_vecPath.begin() + indexEnd;
         iter != iEnd; ++iter)
    {
        drawPath(*iter);
    }
...
ProgramObject& GPUPainterLayer::setPainterAttrib(PathAttrib& attrib)
{
    ...
    switch (attrib.kind)
    {
        ...
        case PathAttrib::PATH_ERASER:
            CGE_LOG_ERROR(
                "GPUPainterLayer::setPainterAttrib - Check your code! 'PATH_ERASER' has unique usage. These codes should never be reached!\n");  // Fall through
        default:
...
void GPUPainterLayer::genInterpolationPoints(const std::vector<Vec2f>& srcPoints, std::vector<Vec2f>& resultPoints,
                                             Vec2f& canvasSize, GLfloat stride)
{
...
void GPUPainterLayer::genInterpolationPointsAndDirections(const std::vector<Vec2f>& srcPoints,
                                                          std::vector<Vec2f>& resultPoints,
                                                          std::vector<Vec2f>& dirPoints, Vec2f& canvasSize,
                                                          GLfloat stride)
{
...
void GPUPainterLayer::genInterpolationPointsAndRandomDir(const std::vector<Vec2f>& srcPoints,
                                                         std::vector<Vec2f>& resultPoints, std::vector<Vec1f>& dir,
                                                         Vec2f& canvasSize, GLfloat stride)
{
    ...
    for (std::vector<Vec2f>::size_type t = 1; t < sz; ++t)
    {
        ...
        const Vec2f dd = d / (float)count;
        const Vec2f startPoint = srcPoints[t - 1] * canvasSize;
        const float seed = (startPoint[0] + startPoint[1]) * float(RAND_MAX);

        // Generate random numbers
        std::mt19937 mt(seed);
        std::uniform_real_distribution<float> dis(0.0f, M_PI);

        for (int i = 0; i < count; ++i)
        {
            resultPoints.push_back(startPoint + dd * (float)i);
            dir.push_back(dis(mt));
        }
    }
}
...
{
    if (m_blendMode == m_programBlendMode && m_blendProgram != nullptr)
    {
        m_blendProgram->bind();
...
{
    if (index >= m_vecPath.size()) return;
...
void GPUPainterMosaicLayer::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    handler->setAsTarget();
    usePainterBlendMode();
    ...
    glBindTexture(GL_TEXTURE_2D, m_canvasTex);
    m_blendProgram->sendUniformi(paramBlendTextureName, 1);
