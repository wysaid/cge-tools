...
namespace CGE
{
	...
	GLuint GPUPainterTextureManager::loadBrushTexture(PathAttrib::BrushStyle style)
	{
		...
		if(name == nullptr || loadFunc == nullptr)
			return 0;
		void* buffer = nullptr;
		int w, h;
		CGEBufferFormat fmt;
		...
		if(unloadFunc != nullptr)
			unloadFunc(loadRet, unloadParam);
		return texID;
	}
	...
	GPUPainterLayer::GPUPainterLayer() : m_draw2ScreenProgram(nullptr), m_canvasTex(0), m_pathIndex(0), m_blendMode(CGE_BLEND_MIX), m_opacity(1.0f), m_blendProgram(nullptr), m_canvasSize(0.0f, 0.0f)
	{
	...
	bool GPUPainterLayer::resizeCanvas(int width, int height, const void* buffer, CGEBufferFormat format)
	{
		...
		if(m_canvasSize[0] == width && m_canvasSize[1] == height && buffer == nullptr && m_canvasTex != 0)
			return true;
