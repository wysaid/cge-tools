...
static CGEConstString s_colorBalance = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform sampler2D inputImageTexture; uniform float redShift; uniform float greenShift; uniform float blueShift;
    ...
    void main() {
        ...
        vec3 midtones =
            (clamp((lightness - b) / a + 0.5, 0.0, 1.0) * clamp((lightness + b - 1.0) / -a + 0.5, 0.0, 1.0) * scale) *
            shift;
