...
static CGEConstString s_vshDeform =
    CGE_SHADER_STRING(attribute vec2 vPosition; attribute vec2 vTexture; varying vec2 textureCoordinate; void main() {
...
static CGEConstString s_fshDeform =
...
static CGEConstString s_fshMesh =
    CGE_SHADER_STRING_PRECISION_L(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; void main() {
...
{
    if (!initShadersFromString(s_vshDeform, s_fshDeform)) return false;

    stride = CGE_MID(stride, 0.001f, 0.2f);

    CGE_LOG_CODE(if (ratio < 0.0f) {
    ...
    CGE_LOG_CODE(
        if (m_meshSize.width < 2 || m_meshSize.width > 5000 || m_meshSize.height < 2 || m_meshSize.height > 5000) {
...
bool CGELiquidationFilter::initBuffers()
{
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, meshIndexes.size() * sizeof(meshIndexes[0]), meshIndexes.data(),
                 GL_STATIC_DRAW);
...
void CGELiquidationFilter::forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity)
{
    ...
    for (int i = 0; i < m_meshSize.height; ++i)
    {
        ...
        for (int j = 0; j < m_meshSize.width; ++j)
        {
            ...
            if (v0[0] < loopStartX || v0[0] > loopEndX || v0[1] < loopStartY || v0[1] > loopEndY) continue;
    ...
    CGE_LOG_CODE(CGE_LOG_INFO("##########Deform mesh take time: %gs #####\n", float(clock() - t) / CLOCKS_PER_SEC););
}

void CGELiquidationFilter::pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity,
                                              float angle)
{
...
bool CGELiquidationFilter::pushMesh()
{
    ...
    if (m_vecMeshes.size() > m_undoSteps) m_vecMeshes.erase(m_vecMeshes.begin(), m_vecMeshes.end() - m_undoSteps);
...
void CGELiquidationNicerFilter::forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
                                                  float intensity)
{
    ...
    // Line equation coefficients, the line is defined by the start and end points.
    ...
    for (int i = 0; i < m_meshSize.height; ++i)
    {
        ...
        for (int j = 0; j < m_meshSize.width; ++j)
        {
            ...
            if (v[0] < loopStartX || v[0] > loopEndX || v[1] < loopStartY || v[1] > loopEndY) continue;
            ...
            // Distance from point to endpoints
            float dis2 = (v - start).length();
            float dis3 = (v - end).length();
            // Projected coordinate of point along the line direction
            const Vec2f projV((eqB * eqB * v[0] - eqA * eqB * v[1] - eqA * eqC) / eqD2,
                              (eqA * eqA * v[1] - eqB * eqC - eqA * eqB * v[0]) / eqD2);

            // Actual distance: use point-to-line distance if the projection is on segment, otherwise use nearer
            // endpoint distance
            float dis;

            if (projV[0] < v2Min[0] || projV[0] > v2Max[0] || projV[1] < v2Min[1] || projV[1] > v2Max[1])
            {
...
void CGELiquidationNicerFilter::pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
