 ...
 void main()
{
    ...
    if(disToLine < radius)
    {
        ...
        // Compute actual distance: use point-to-line distance if projection is on segment, otherwise use nearest endpoint distance
        float percent;
...
namespace CGE
{
    CGEDistortionFilter::CGEDistortionFilter() : m_intensityLoc(-1), m_radiusLoc(-1), m_stepsLoc(-1), m_keyPointLoc(-1), m_keyPoint2Loc(-1)
    {
    ...
    void CGEDistortionFilter::setForwardParams(float point1X, float point1Y, float point2X, float point2Y, float radius, float intensity)
    {
        m_program.bind();
