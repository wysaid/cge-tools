...
{
	vec4 src = texture2D(inputImageTexture, textureCoordinate);
	vec4 dst = texture2D(blendTexture, fract(textureCoordinate * scalingRatio)) * intensity;
	gl_FragColor = vec4(blend(src.rgb, dst.rgb, dst.a), src.a);
});
...
vec3 blend(vec3 src1, vec3 src2, float alpha)
{
    ...
    else src2.r = (src1.r - 1.0) * (alpha - 2.0 * src2.r) + src1.r;
    
    if(src1.g < 0.5)
        src2.g = (src1.g * src2.g) * 2.0 + src1.g * revAlpha;
    else src2.g = (src1.g - 1.0) * (alpha - 2.0 * src2.g) + src1.g;
    
    if(src1.b < 0.5)
        src2.b = (src1.b * src2.b) * 2.0 + src1.b * revAlpha;
    else src2.b = (src1.b - 1.0) * (alpha - 2.0 * src2.b) + src1.b;
