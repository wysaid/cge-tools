...
static CGEConstString s_blendHelpStart =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D blendTexture; uniform float intensity;

                                  % s\n

                                  void main() {
                                      vec4 src = texture2D(inputImageTexture, textureCoordinate);
                                      vec4 dst = texture2D(blendTexture, textureCoordinate) * intensity;
                                      gl_FragColor = vec4(blend(src.rgb, dst.rgb, dst.a), src.a);
                                  });

static CGEConstString s_blendKeepRatioHelpStart = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D blendTexture;
    uniform float intensity; uniform vec4 ratioAspect;

    % s\n

    void main() {
        vec4 src = texture2D(inputImageTexture, textureCoordinate);
        vec4 dst = texture2D(blendTexture, (textureCoordinate * ratioAspect.xy) + ratioAspect.zw) * intensity;
        gl_FragColor = vec4(blend(src.rgb, dst.rgb, dst.a), src.a);
    });

static CGEConstString s_blendTileHelpStart =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D blendTexture; uniform float intensity; uniform vec2 scalingRatio;
                                  ...
                                  void main() {
                                      vec4 src = texture2D(inputImageTexture, textureCoordinate);
                                      vec4 dst =
                                          texture2D(blendTexture, fract(textureCoordinate * scalingRatio)) * intensity;
                                      gl_FragColor = vec4(blend(src.rgb, dst.rgb, dst.a), src.a);
                                  });

static CGEConstString s_blendPixelHelpStart = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec4 blendColor;
    ...
    void main() {
        vec4 src = texture2D(inputImageTexture, textureCoordinate);
        gl_FragColor = vec4(blend(src.rgb, blendColor.rgb * intensity, intensity * blendColor.a), src.a);
    });
...
static CGEConstString s_blendVignetteHelpStart =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform vec4 blendColor; uniform float intensity;

                                  uniform vec2 vignette; uniform vec2 vignetteCenter;
                                  ...
                                  void main() {
                                      ...
                                      vec4 dstColor = blendColor * (intensity * percent);
                                      gl_FragColor = vec4(blend(src.rgb, dstColor.rgb, dstColor.a), src.a);
                                  });

static CGEConstString s_blendVignette2HelpStart =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform vec4 blendColor; uniform float intensity;
...
static CGEConstString s_blendVignetteNoAlphaHelpStart =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform vec4 blendColor; uniform float intensity;
                                  ...
                                  void main() {
                                      ...
                                      vec4 dstColor = blendColor * intensity;
                                      gl_FragColor = vec4(blend(src.rgb, dstColor.rgb * alpha, dstColor.a), src.a);
                                  });

static CGEConstString s_blendVignette2NoAlphaHelpStart =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform vec4 blendColor; uniform float intensity;
...
static CGEConstString s_hardLight = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    if (src2.r < 0.5)
...
static CGEConstString s_overLay = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    float revAlpha = 1.0 - alpha;
    // ks version.
    if (src1.r < 0.5)
        src2.r = (src1.r * src2.r) * 2.0 + src1.r * revAlpha;
    else
        src2.r = (src1.r - 1.0) * (alpha - 2.0 * src2.r) + src1.r;

    if (src1.g < 0.5)
        src2.g = (src1.g * src2.g) * 2.0 + src1.g * revAlpha;
    else
        src2.g = (src1.g - 1.0) * (alpha - 2.0 * src2.g) + src1.g;

    if (src1.b < 0.5)
        src2.b = (src1.b * src2.b) * 2.0 + src1.b * revAlpha;
    else
        src2.b = (src1.b - 1.0) * (alpha - 2.0 * src2.b) + src1.b;

    return src2;
});

static CGEConstString s_softLight = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    return mix(src1, src1 * (alpha * src1 + (2.0 * src2 * (1.0 - src1))) + src1 * (1.0 - alpha), alpha);
});

static CGEConstString s_screen = CGE_SHADER_STRING(
    vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, src1 + src2 - src1 * src2, alpha); });

static CGEConstString s_linearLight = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    return mix(src1, clamp(src1 + src2 * 2.0 - 1.0, 0.0, 1.0), alpha);
});

static CGEConstString s_vividLight = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
...
static CGEConstString s_multiply =
    CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) { return src1 * (1.0 - alpha) + src1 * src2; });

static CGEConstString s_exclude = CGE_SHADER_STRING(
    vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, src1 + src2 - src1 * src2 * 2.0, alpha); });

static CGEConstString s_darken =
    CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, min(src1, src2), alpha); });

static CGEConstString s_lighten =
    CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) { return max(src1, src2); });

static CGEConstString s_colorBurn = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
...
static CGEConstString s_colorDodge = CGE_SHADER_STRING(
    vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, min(src1 / (1.0 - src2), 1.0), alpha); });
...
static CGEConstString s_linearDodge = CGE_SHADER_STRING(
    vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, min(src1 + src2, 1.0), alpha); });

static CGEConstString s_linearBurn = CGE_SHADER_STRING(
    vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, max(src1 + src2 - 1.0, 0.0), alpha); });

static CGEConstString s_pinLight = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    src2 *= 2.0;
    if (src2.r > src1.r)
    {
        src2.r = src2.r - 1.0;
        if (src2.r < src1.r) src2.r = src1.r;
    }
    if (src2.g > src1.g)
    {
        src2.g = src2.g - 1.0;
        if (src2.g < src1.g) src2.g = src1.g;
    }
    if (src2.b > src1.b)
    {
        src2.b = src2.b - 1.0;
        if (src2.b < src1.b) src2.b = src1.b;
    }
...
static CGEConstString s_hardMix = CGE_SHADER_STRING(
    vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, step(1.0 - src1, src2), alpha); });

static CGEConstString s_difference =
    CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, abs(src1 - src2), alpha); });

static CGEConstString s_add = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    return src1 + src2 * alpha;  // mix(src1, src1 + src2, alpha);
});

static CGEConstString s_mix =
    CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, src2, alpha); });

static CGEConstString s_color = CGE_SHADER_STRING(
    float getLumValue(vec3 src) { return 0.299 * src.r + 0.587 * src.g + 0.114 * src.b; }

    vec3 blend(vec3 src1, vec3 src2, float alpha) {
        return mix(src1, getLumValue(src1) - getLumValue(src2) + src2, alpha);
    });

static CGEConstString s_addReverse =
    CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) { return src1 * (1.0 - alpha) + src2; });

static CGEConstString s_colorBW = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
...
static CGEConstString s_darkerColor = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    vec3 dst = dot(src1, vec3(0.299, 0.587, 0.114)) < dot(src2, vec3(0.299, 0.587, 0.114)) ? src1 : src2;
    return mix(src1, dst, alpha);
});

static CGEConstString s_lighterColor = CGE_SHADER_STRING(vec3 blend(vec3 src1, vec3 src2, float alpha) {
    vec3 dst = dot(src1, vec3(0.299, 0.587, 0.114)) > dot(src2, vec3(0.299, 0.587, 0.114)) ? src1 : src2;
    return mix(src1, dst, alpha);
});

static CGEConstString s_subtract = CGE_SHADER_STRING(
    vec3 blend(vec3 src1, vec3 src2, float alpha) { return mix(src1, max(src1 - src2, 0.0), alpha); });
...
static CGEConstString s_hue = CGE_SHADER_STRING(
    ...
    {
        if (src.y <= 0.0) return vec3(src.z, src.z, src.z);
...
static const char* s_shaderEnum[] = {
    s_mix,       // 0 Normal
    s_dissolve,  // 1 Dissolve

    s_darken,       // 2 Darken
    s_multiply,     // 3 Multiply
    s_colorBurn,    // 4 Color Burn
    s_linearBurn,   // 5 Linear Burn
    s_darkerColor,  // 6 Darker Color

    s_lighten,       // 7 Lighten
    s_screen,        // 8 Screen
    s_colorDodge,    // 9 Color Dodge
    s_linearDodge,   // 10 Linear Dodge
    s_lighterColor,  // 11 Lighter Color

    s_overLay,      // 12 Overlay
    s_softLight,    // 13 Soft Light
    s_hardLight,    // 14 Hard Light
    s_vividLight,   // 15 Vivid Light
    s_linearLight,  // 16 Linear Light
    s_pinLight,     // 17 Pin Light
    s_hardMix,      // 18 Hard Mix

    s_difference,  // 19 Difference
    s_exclude,     // 20 Exclusion
    s_subtract,    // 21 Subtract
    s_divide,      // 22 Divide
    s_hue,         // 23 Hue
    s_saturation,  // 24 Saturation
    s_color,       // 25 Color
    s_luminosity,  // 26 Luminosity
...
{
    if (texID == m_blendTexture) return;
    if (shouldDelete) glDeleteTextures(1, &m_blendTexture);
...
bool CGEBlendKeepRatioFilter::initWithMode(CGETextureBlendMode mode)
{
    ...
    {
        initSampler();
        if (m_uniformParam != nullptr) m_uniformParam->requireRatioAspect(paramAspectRatioName);
        return true;
    }
    CGE_LOG_ERROR("Init CGEBlendFilter failed, Mode number %d\n", mode);
    return false;
...
void CGEBlendTileFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    ...
    m_program.sendUniformf(paramScalingRatioName, sz.width / (float)m_blendTextureSize.width,
                           sz.height / (float)m_blendTextureSize.height);

    if (m_uniformParam != nullptr) m_uniformParam->assignUniforms(handler, m_program.programID());
    glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
    CGE_LOG_CODE(cgeCheckGLError("glDrawArrays");)
}
