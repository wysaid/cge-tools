...
namespace CGE
{
	...
	private:
		...
		void blur_tex(CGE::ProgramObject &shader,GLuint srcTexture,GLuint disTexture,CGESizei size,GLuint vertexBufferID);
...
//		m_srcTex(-1),
	...
	void CGEFastTiltShift::draw_tex(GLuint srcTexture,GLuint dstTexture,CGESizei dstSize,float wf,float hf,GLuint vertexBufferID)
	{
		glViewport(0, 0, dstSize.width, dstSize.height);
	...
	void CGEFastTiltShift::draw_tmp_tex(GLuint srcTexture,GLuint vertexBufferID)
	{
		...
		draw_tex(m_tmpTexs[idx + 1][0],m_tmpTexs[idx][0],m_texSizes[idx],m_wFac[idx % 2] / m_wFac[(idx + 1) % 2],m_hFac[idx % 2] / m_hFac[(idx + 1) % 2],0);
		blur_tex(m_b1Shader,m_tmpTexs[idx][0],m_tmpTexs[idx][1],m_texSizes[idx],0);
		glBindTexture(GL_TEXTURE_2D, m_tmpTexs[idx + 1][0]);
	...
	inline void init_tex(std::vector<GLuint> &tex,CGESizei size)
	{
		...
		for(int i=0;i<3;++i)
		{
			GLuint texID = cgeGenTextureWithBuffer(nullptr , size.width, size.height, GL_RGBA, GL_UNSIGNED_BYTE, 4, 0, GL_NEAREST);
			tex.push_back(texID);
		}
	}
	
	void CGEFastTiltShift::blur_tex(CGE::ProgramObject &shader,GLuint srcTexture,GLuint dstTexture,CGESizei size,GLuint vertexBufferID)
	{
