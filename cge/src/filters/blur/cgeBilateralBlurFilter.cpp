...
CGEConstString s_fshBilateralBlur = CGE_SHADER_STRING_PRECISION_H(
    ...
    uniform float distanceNormalizationFactor; uniform float blurSamplerScale; uniform vec2 samplerSteps;
    ...
    void main() {
        ...
        for (int i = 0; i < samplerRadius; ++i)
        {
...

            {
                ...
                float distanceFromCentralColor1 =
                    min(distance(centralColor, sampleColor1) * distanceNormalizationFactor, 1.0);
...

            {
                ...
                float distanceFromCentralColor2 =
                    min(distance(centralColor, sampleColor2) * distanceNormalizationFactor, 1.0);
...
void CGEBilateralWrapperFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                               GLuint vertexBufferID)
{
    assert(m_proc != nullptr);  // Filter has not been initialized successfully
