...
static CGEConstString s_vshFaceBlend =
    CGE_SHADER_STRING(attribute vec2 vPosition;    // Target face position
                      attribute vec2 srcPosition;  // Original face position
                      attribute float posDis;      // Progressive blend weight

                      varying vec2 srcTexCoord; varying vec2 dstTexCoord; varying float srcWeight;
...
static CGEConstString s_fshFaceBlend = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform sampler2D srcTex; uniform sampler2D dstTex; uniform float srcAvg; uniform float dstAvg;
...
CGEFaceBlendFilter::CGEFaceBlendFilter() :
    m_srcFaceVertexVBO(0),
    m_dstFaceVertexVBO(0),
    m_faceGradientVBO(0),
    m_faceMeshIndexVBO(0),
    m_cacheTexture(0),
    m_drawer(nullptr)
{
...
bool CGEFaceBlendFilter::initWithBlendMode(CGETextureBlendMode blendMode)
{
    ...
    if (blendFuncStr == nullptr) return false;
    ...
    if (initShadersFromString(s_vshFaceBlend, buffer))
    {
        ...
        if (m_drawer == nullptr) m_drawer = TextureDrawer::create();
...
void CGEFaceBlendFilter::_genFaceBuffers()
{
    ...
    // mesh indices
    {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, meshIndices.size() * sizeof(meshIndices[0]), meshIndices.data(),
                     GL_STATIC_DRAW);
...
double CGEFaceBlendFilter::getAverageLumByTexture(const FrameBuffer& framebuffer, GLuint texture, const Vec4f& area,
                                                  const Vec2f& texSize)
{
...
{
    CGE_LOG_CODE(if (m_drawer == nullptr) {
