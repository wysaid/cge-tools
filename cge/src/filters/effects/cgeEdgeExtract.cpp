...
namespace CGE
{
	...
	private:
		...
		void size_upCompressOdd(GLuint srcTexture,GLuint cpTexture,GLuint dstTexture,CGESizei dstSize,GLuint vertexBufferID);

		void size_upCompressEvn(GLuint srcTexture,GLuint cpTexture,GLuint dstTexture,CGESizei dstSize,GLuint vertexBufferID);
	...
	void CGEEdgeExtract::size_upCompressOdd(GLuint srcTexture,GLuint cpTexture,GLuint dstTexture,CGESizei dstSize,GLuint vertexBufferID)
	{
		glViewport(0, 0, dstSize.width,dstSize.height);
	...
	void CGEEdgeExtract::size_upCompressEvn(GLuint srcTexture,GLuint cpTexture,GLuint dstTexture,CGESizei dstSize,GLuint vertexBufferID)
	{
		glViewport(0, 0, dstSize.width,dstSize.height);
	...
	void CGEEdgeExtract::draw_tex(GLuint srcTexture,GLuint dstTexture,CGESizei dstSize,float wf,float hf,GLuint vertexBufferID)
	{
		glViewport(0, 0, dstSize.width, dstSize.height);
	...
	{
		return cgeGenTextureWithBuffer(nullptr , size.width, size.height, GL_RGBA, GL_UNSIGNED_BYTE, 4, 0, nearest ? GL_NEAREST : GL_LINEAR);
	}
	...
	void CGEEdgeExtract::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
	{
		...
		draw_tex(m_tmpTexs.back()[sourcIdx],handler->getTargetTextureID(),handler->getOutputFBOSize(),1.0 / m_wFac,1.0 / m_hFac,vertexBufferID);

		//draw_tex(m_tmpTexs[level + 1][0],handler->getTargetTextureID(),handler->getOutputFBOSize(),1.0 / m_wFac,1.0 / m_hFac,vertexBufferID);
