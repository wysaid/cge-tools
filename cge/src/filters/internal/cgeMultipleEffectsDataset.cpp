...
1. Each processing step starts with '@' followed by the method name. Spaces between are optional.
   Example: "@ method" or "@method" are both valid. The method may be followed by any number of space-separated arguments.
   All characters after the method name (up to the next "@" symbol or end of string) are passed to the corresponding Parser.
...
2. curve method: format "@curve <arg1> <arg2> ... <argN>"
   ...
   Spaces between parentheses and parameters are optional. x and y values within parentheses may be separated by any delimiter such as spaces or commas.
   ...
   Note: When making consecutive curve adjustments, you only need to write "@curve" once. If curves A and B have no operations in between, they can be merged into a single curve instruction, saving overhead and improving performance.
   Example: "@curve RGB(0,0) (100, 200) (255, 255) R(0,0) (50, 25) (255, 255) G(0, 0) (100,150) (255, 255) RGB(0, 0) (200, 150) (255, 255)" (produces the same result as applying curve A then curve B sequentially)
...
3. blend method: format "@blend <function> <texture> <intensity>"
   ...
   Add:              add
   Add Reverse:      addrev
   Black & White:    colorbw  // This method replaces the source color with black/white based solely on the alpha channel of the texture.
   ...
   Since square brackets cannot appear in ordinary filenames, this avoids conflicts. Note: this advanced usage is for programmers only and is not suitable for editors.
   IMPORTANT: An incorrect texture id in advanced usage will conflict with other texture ids and can easily break the entire application!
...
5. pixblend method: format "@pixblend <function> <color> <intensity>"
   ...
   Note: color values can be decimals. When the alpha value is greater than 1, all color values are treated as [0, 255]; otherwise [0, 1].

6. selfblend method: format "@selfblend <function> <intensity>"
   Note: Parameters are the same as blend, except there is no <texture> parameter. This method blends the image with its own colors.

7. adjust method: format "@adjust <function> <arg1>...<argN>"
   Available <function> values:
   brightness: one parameter intensity, range [-1, 1]

   contrast: one parameter intensity > 0; intensity=0 produces a gray image, intensity=1 is no effect, intensity>1 increases contrast.

   saturation: one parameter intensity > 0; intensity=0 produces a monochrome image, intensity=1 is no effect, intensity>1 increases saturation.
   ...
   shadowhighlight[shl]: two parameters shadow (range [-200, 100], 0=no effect) and highlight (range [-100, 200], 0=no effect).
   ...
   lut (lookup table): provides a general LUT filter solution. Format: "@lookup <lut_image_name>", where lut_image_name is the lookup image filename.

   Note: Values in [] are abbreviations. <arg*> is the required parameter; see the relevant class for value ranges. The number of <arg*> depends on the specific <function>.

8. cvlomo method: includes the curve sub-method. Format "@cvlomo <vignetteStart> <vignetteEnd> <colorScaleLow> <colorScaleRange> <saturation> <curve>"
   ...
   <curve> is a complete curve method without the @curve tag.
   Example: "@cvlomo 0.2 0.8 0.1 0.2 1 RGB(0, 0) (255, 255)"
...
10. vignette method: format "@vignette <low> <range> <centerX> <centerY>"
    Note: low and range are required; centerX and centerY are optional; if omitted, they default to 0.5. Both must be present to take effect.
    Example: "@vignette 0.1 0.9", "@vignette 0.1 0.9 0.5 0.5"
...
16. style method: format "@style <function> <arg1> ... <argN>"
    ...
    haze: three parameters distance range[-0.5, 0.5], slope range[-0.5, 0.5] and color (three components r, g, b, all in [0, 1])
    ...
    "colorMapping" can be abbreviated as "cm". imageFile is the mapping image. mappingWidth/Height is the resolution used for mapping. unitWidth/Height is the per-unit resolution (grid cells must be uniform with no gaps).
