...
Overview:
The purpose of ParsingEngine is to let even people with no GLSL knowledge know how to quickly add new effects.


EffectString parsing rules and common method descriptions:

1. Each processing step starts with '@' followed by the method name. Spaces between are optional.
   Example: "@ method" or "@method" are both valid. The method may be followed by any number of space-separated
arguments. All characters after the method name (up to the next "@" symbol or end of string) are passed to the
corresponding Parser.

2. curve method: format "@curve <arg1> <arg2> ... <argN>"
   <argN> has two formats: "RGB (x1,y1) (xn, yn)" where xn and yn are numbers in [0, 255],
   or "R (rx1,ry1) ... (rxn, ryn) G (gx1,gy1) ... (gxn,gyn) B (bx1,by1)...(bxn,byn)"
   where R, G, B denote their respective channels, followed by the control points for that channel.
   Spaces between parentheses and parameters are optional. x and y values within parentheses may be separated by any
delimiter such as spaces or commas.

   Example:
   Curve A: RGB channel control points are (0, 0) (100, 200) (255, 255)
   Format: "@curve RGB(0,0) (100, 200) (255, 255)"

   Curve B: R channel (0, 0) (50, 25) (255, 255),
            G channel (0, 0) (100, 150) (255, 255),
            RGB channel (0, 0) (200, 150) (255, 255)
   Format: "@curve R(0,0) (50, 25) (255, 255) G(0, 0) (100,150) (255, 255) RGB(0, 0) (200, 150) (255, 255)"
   (Note: A curve formatting tool is available in the tools directory.)

   Note: When making consecutive curve adjustments, you only need to write "@curve" once. If curves A and B have no
operations in between, they can be merged into a single curve instruction, saving overhead and improving performance.
   Example: "@curve RGB(0,0) (100, 200) (255, 255) R(0,0) (50, 25) (255, 255) G(0, 0) (100,150) (255, 255) RGB(0, 0)
(200, 150) (255, 255)" (produces the same result as applying curve A then curve B sequentially)

3. blend method: format "@blend <function> <texture> <intensity>"
   Available <function> values:

   Normal:           mix
   Dissolve:         dissolve[dsv]

   Darken:           darken[dk]
   Multiply:         multiply[mp]
   Color Burn:       colorburn[cb]
   Linear Burn:      linearburn[lb]
   Darker Color:     darkercolor[dc]

   Lighten:          lighten[lt]
   Screen:           screen[sr]
   Color Dodge:      colordodge[cd]
   Linear Dodge:     lineardodge[ld]
   Lighter Color:    lightercolor[lc]

   Overlay:          overlay[ol]
   Soft Light:       softlight[sl]
   Hard Light:       hardlight[hl]
   Vivid Light:      vividlight[vvl]
   Linear Light:     linearlight[ll]
   Pin Light:        pinlight[pl]
   Hard Mix:         hardmix[hm]
   ...
   Add:              add
   Add Reverse:      addrev
   Black & White:    colorbw  // This method replaces the source color with black/white based solely on the alpha
channel of the texture.

   Note: Values in [] are abbreviations that can be used instead of the full parameter name.

   <texture> is the resource filename including extension!

   Note (added 2016-1-5): The <texture> parameter can also be a texture id,width,height
   (enclosed in brackets, comma-separated, no spaces, e.g. [10,1024,768] means texture id 10, 1024px wide, 768px tall).
   Since square brackets cannot appear in ordinary filenames, this avoids conflicts. Note: this advanced usage is for
programmers only and is not suitable for editors. IMPORTANT: An incorrect texture id in advanced usage will conflict
with other texture ids and can easily break the entire application!

   <intensity> is the blend strength (opacity), an integer in the range (0, 100].

   Example: blend with resource image src.jpg at 80% strength
   Format: "@blend overlay src.jpg 80" or "@blend ol src.jpg 80"

4. krblend method: format is the same as blend. See @blend.
   All parameters and usage are identical to blend. The difference is that krblend preserves the texture's aspect ratio
   and scales it proportionally so that it covers the entire image.

5. pixblend method: format "@pixblend <function> <color> <intensity>"
   <function> is the same as in blend; see blend.
   <intensity> has the same meaning as in blend; see blend.
   <color> contains four floats for r, g, b, a in the range [0, 1] or [0, 255].
   Example: blend with solid red at 90% strength
   Format: "@pixblend overlay 1 0 0 0 90"
   Note: color values can be decimals. When the alpha value is greater than 1, all color values are treated as [0, 255];
otherwise [0, 1].

6. selfblend method: format "@selfblend <function> <intensity>"
   Note: Parameters are the same as blend, except there is no <texture> parameter. This method blends the image with its
own colors.

7. adjust method: format "@adjust <function> <arg1>...<argN>"
   Available <function> values:
   brightness: one parameter intensity, range [-1, 1]

   contrast: one parameter intensity > 0; intensity=0 produces a gray image, intensity=1 is no effect, intensity>1
increases contrast.

   saturation: one parameter intensity > 0; intensity=0 produces a monochrome image, intensity=1 is no effect,
intensity>1 increases saturation.

   monochrome: six parameters, range [-2, 3], consistent with Photoshop. Order: red, green, blue, cyan, magenta, yellow.

   sharpen: one parameter intensity, range [0, 10]; intensity=0 means no effect.
   blur: one parameter intensity, range [0, 1]; intensity=0 means no effect.

   whitebalance: two parameters temperature (range [-1, 1], 0=no effect) and tint (range [0, 5], 1=no effect).

   shadowhighlight[shl]: two parameters shadow (range [-200, 100], 0=no effect) and highlight (range [-100, 200], 0=no
effect).

   hsv: six parameters red, green, blue, magenta, yellow, cyan. All in range [-1, 1].
   hsl: three parameters hue, saturation, luminance. All in range [-1, 1].

   level: three parameters dark, light, gamma. All in range [0, 1], where dark < light.

   exposure: one parameter intensity, range [-10, 10].

   colorbalance: three parameters redShift [-1, 1], greenShift [-1, 1], blueShift [-1, 1]. (Added: 2015-3-30)

   lut (lookup table): provides a general LUT filter solution. Format: "@lookup <lut_image_name>", where lut_image_name
is the lookup image filename.

   Note: Values in [] are abbreviations. <arg*> is the required parameter; see the relevant class for value ranges. The
number of <arg*> depends on the specific <function>.

8. cvlomo method: includes the curve sub-method. Format "@cvlomo <vignetteStart> <vignetteEnd> <colorScaleLow>
<colorScaleRange> <saturation> <curve>" <vignetteStart> and <vignetteEnd> are decimals greater than 0, typically less
than 1. <colorScaleLow> and <colorScaleRange> are decimals greater than 0, typically less than 1. Used to adjust the
image. <saturation> is a decimal in [0, 1]. Sets the image saturation. <curve> is a complete curve method without the
@curve tag. Example: "@cvlomo 0.2 0.8 0.1 0.2 1 RGB(0, 0) (255, 255)"

9. colorscale method: format "@colorscale <low> <range> <saturation>"
   Note: colorscale requires CPU computation and may affect performance.

10. vignette method: format "@vignette <low> <range> <centerX> <centerY>"
    Note: low and range are required; centerX and centerY are optional; if omitted, they default to 0.5. Both must be
present to take effect. Example: "@vignette 0.1 0.9", "@vignette 0.1 0.9 0.5 0.5"

11. colormul method: format "@colormul <function> <arg1> ...<argN>"
    <function> currently supports "flt", "vec", and "mat".
    When "flt": followed by one <arg>; multiplies all pixels by a scalar.
    When "vec": followed by three args <arg1> <arg2> <arg3>; multiplies each RGB component separately.
    When "mat": followed by nine args; performs a 3x3 matrix multiplication on each pixel's RGB.

12. special method: format "@special <N>"
    <N> is the effect number.
    Used for all non-generic effects; a dedicated processor is written directly.

13. lomo method: format "@lomo <vignetteStart> <vignetteEnd> <colorScaleLow> <colorScaleRange> <saturation> <isLinear>"
    <vignetteStart> and <vignetteEnd> are decimals greater than 0, typically less than 1.
    <colorScaleLow> and <colorScaleRange> are decimals greater than 0, typically less than 1. Used to adjust the image.
    <saturation> is a decimal in [0, 1]. Sets the image saturation.
    <isLinear> 0 or 1. Sets whether the vignette uses linear growth. Defaults to 0 if omitted.
    Example: "@lomo 0.2 0.8 0.1 0.2 1 0"

====== Effects up to #13 require library version: 0.2.1.x =========

14. vigblend method: format "@vigblend <function> <color> <intensity> <low> <range> <centerX> <centerY> [kind]"
    [isLinear] is optional, defaults to 0.
    0: linear enhancement, vignette has no alpha channel (alpha=1)
    1: linear enhancement, vignette gradient uses alpha channel
    2: quadratic enhancement, vignette has no alpha channel (alpha=1)
    3: quadratic enhancement, vignette gradient uses alpha channel
    Example: "@vigblend ol 0 0 0 1 50 0.02 0.45 0.5 0.5 0"
    ...
    Note: other parameter meanings and usage refer to the pixblend and vignette methods.


====== Effects above this comment require library version 0.3.2.1

15. selcolor (selective color) method: format "@selcolor <color1> <colorargs1> ...<colorN> <colorargsN>"
    <colorN> is the color to select from; valid values:
    red, green, blue, cyan, magenta, yellow, white, gray, black.

    <colorargsN> is the adjustment applied to the selected color, in the format:
    (cyan, magenta, yellow, key)   range: [-1, 1]
    Each <colorargsN> is four parameters enclosed in parentheses. Write 0 for any unadjusted parameter.

====== Added 2014-11-12

16. style method: format "@style <function> <arg1> ... <argN>"
    Available <function> values:

    crosshatch: two parameters spacing range[0, 0.1] and lineWidth range(0, 0.01]

    edge (Sobel edge detection): two parameters mix range[0, 1] and stride range(0, 5]

    emboss: three parameters mix range[0, 1], stride range[1, 5] and angle range[0, 2π]

    halftone: one parameter dotSize >= 1

    haze: three parameters distance range[-0.5, 0.5], slope range[-0.5, 0.5] and color (three components r, g, b, all in
[0, 1])

    polkadot: one parameter dotScaling range(0, 1]

    sketch: one parameter intensity [0, 1]

    max (maximum value): no parameters

    min (minimum value): no parameters

    mid (median value): no parameters

    colorMapping:
    Format 1: "@style colorMapping <imageFile> <mappingWidth> <mappingHeight> <unitWidth> <unitHeight>"
    "colorMapping" can be abbreviated as "cm". imageFile is the mapping image. mappingWidth/Height is the resolution
used for mapping. unitWidth/Height is the per-unit resolution (grid cells must be uniform with no gaps).

====== Added 2015-2-5

17. beautify method: format "@beautify <function> <arg1>...<argN>"

    Available <function> values:

    bilateral: three parameters blurScale range[-100, 100], distanceFactor range[1, 20], and repeat times >= 1.
               repeat times is optional; defaults to 1.

    face: one or three parameters — intensity range(0, 1] (0 has no effect), photo width, photo height.
          If width/height are omitted, they default to 720x1280.

====== Added 2015-3-19

18. New functional method: unpack. Add "#unpack" at the very beginning of the configuration.
    Effect: removes the MultipleEffects wrapper and adds all parsed effects directly to the entire handler.

====== Added 2015-8-7

19. blur method:
    New @blur method, dedicated to various blur algorithms. Format: "@blur <function> <arg1> ... <argN>"

    Available <function> values:

    lerp (lerp blur): two parameters blur level[0, 1], blur base [0.6, 2.0]


====== Added 2015-8-8

20. dynamic method:
    New @dynamic method, dedicated to various dynamic filter effects. Format: "@dynamic <function> <arg1> ... <argN>"
    Note: The effect editor does not support editing dynamic methods.

    wave (dynamic wave, added 2015-11-18): followed by 1, 3, or 4 parameters.
    ① One parameter: dynamic wave. The parameter is autoMotionSpeed (auto update speed). See wave filter definition.
    ② Three parameters: static wave. First is motion, second is angle, third is strength. See wave filter definition.
    ③ Four parameters: dynamic wave. First three same as ②, fourth is autoMotionSpeed.

    In other words, with one parameter, motion, angle, and strength all use their default values.
...
const char* g_cgeEffectString[] =
    {
        ...
        "@pixblend sl 77 103 234 255 100 @blend sl e524_t.jpg 100 @vigblend ol 0 0 0 1 100 0 0.5 0.5 0.5 3",  // 525
                                                                                                              // //"@pixblend
                                                                                                              // sl 77
                                                                                                              // 103 234
                                                                                                              // 255 100
                                                                                                              ...
                                                                                                              // 100",
                                                                                                              // //525
        "@pixblend sl 100 212 225 255 100 @blend sl e526_t.jpg 100 @vigblend ol 0 0 0 1 100 0 0.5 0.5 0.5 3",  // 526
                                                                                                               // //"@pixblend
                                                                                                               // sl 100
                                                                                                               // 212
                                                                                                               // 225
                                                                                                               ...
                                                                                                               // 100",
                                                                                                               // //526
        "@pixblend sl 246 99 241 255 100 @blend sl e526_t.jpg 100 @vigblend ol 0 0 0 1 100 0 0.5 0.5 0.5 3",  // 527
                                                                                                              // //"@pixblend
                                                                                                              // sl 246
                                                                                                              // 99 241
                                                                                                              // 255 100
                                                                                                              ...
                                                                                                              // 100",
                                                                                                              // //527
        "@pixblend sl 241 226 95 255 100 @blend sl e528_t.jpg 100 @vigblend ol 0 0 0 1 40 0 0.5 0.5 0.5 3",  // 528
                                                                                                             // //"@pixblend
                                                                                                             // sl 241
                                                                                                             // 226 95
                                                                                                             // 255 100
                                                                                                             ...
                                                                                                             // 40",
                                                                                                             // //528
        "@pixblend sl 241 164 95 255 100 @blend sl e529_t.jpg 100 @vigblend ol 0 0 0 1 60 0 0.5 0.5 0.5 3",  // 529
                                                                                                             // //"@pixblend
                                                                                                             // sl 241
                                                                                                             // 164 95
                                                                                                             // 255 100
        ...
        "@adjust saturation 0.2 @blend ol e530_t.jpg 100 @vigblend ol 0 0 0 1 60 0 0.5 0.5 0.5 3",  // 530 //"@adjust
                                                                                                    // saturation 0.2
                                                                                                    // @blend ol
                                                                                                    // e530_t.jpg 100
                                                                                                    // @krblend ol
        ...
        "@adjust saturation 0.2 @blend ol e534_t.jpg 100 @vigblend ol 0 0 0 1 40 0 0.5 0.5 0.5 3",  // 534 //"@adjust
                                                                                                    // saturation 0.2
                                                                                                    // @blend ol
                                                                                                    // e534_t.jpg 100
                                                                                                    // @krblend ol
                                                                                                    // vtg410.png 40",
                                                                                                    // //534
        "@adjust saturation 0 @blend ol e535_t.jpg 80@vigblend ol 0 0 0 1 60 0 0.5 0.5 0.5 3",  // 535 //"@adjust
                                                                                                // saturation 0 @blend
                                                                                                // ol e535_t.jpg 80
                                                                                                // @krblend ol
                                                                                                // vtg410.png 60", //535

        "@pixblend sl 85 180 16 255 100 @vigblend ol 0 0 0 1 20 0 0.5 0.5 0.5 3",  // 536 //"@pixblend sl 85 180 16 255
                                                                                   // 100 @krblend ol vtg410.png 20",
                                                                                   // //536
        "@pixblend sl 255 90 0 255 100 @vigblend ol 0 0 0 1 20 0 0.5 0.5 0.5 3",  // 537 //"@pixblend sl 255 90 0 255
                                                                                  // 100 @krblend ol vtg410.png 20",
                                                                                  // //537
        "@adjust saturation 0.4 @pixblend sl 234 201 45 255 100 @vigblend ol 0 0 0 1 20 0 0.5 0.5 0.5 3",  // 538
                                                                                                           // //"@adjust
                                                                                                           // saturation
                                                                                                           // 0.4
                                                                                                           // @pixblend
                                                                                                           ...
                                                                                                           // vtg410.png
                                                                                                           // 20", //538
        "@adjust saturation 0.4 @pixblend sl 106 173 215 255 100 @vigblend ol 0 0 0 1 20 0 0.5 0.5 0.5 3",  // 539
                                                                                                            // //"@adjust
                                                                                                            // saturation
                                                                                                            // 0.4
                                                                                                            // @pixblend
        ...
        ////////////////  ↑ Film additions 2014-11-18  two total/////////////////////
        ////////////////  ↓ New additions bw 2014-12-3 entries 564-582 nineteen total /////////////////////
        ...
        //////////////// ↑ New additions bw 2014-12-3 entries 564-582 nineteen total /////////////////////
        //////////////// ↓ New additions Xmas 2014-12-3 entries 583-593 eleven total ///////////////////
        ...
        //////////////// ↓ New additions Tokyo 2014-12-3 entries 594-608 fifteen total ///////////////////
        ...
        //////////////// ↓ New additions Cinematic 2014-12-3 entries 609-610 two total ///////////////////
...
{
    if (effectID < 0 || effectID >= g_cgeEffectNum) return (const char*)(0);
    return g_cgeEffectString[effectID];
}

}  // namespace CGEEffectsDataSet

// Helper code for saving effect files
// #include <fstream>
// using namespace std;
// bool save2File()
// {
