...
#define BLENDBORDER_PREMULTIPLIED                  \
    "vec3 blend(vec3 src, vec3 dst, float alpha){" \
    "return src * (1.0 - alpha) + dst;}"

#define BLENDBORDER_NOT_PREMULTIPLIED              \
    "vec3 blend(vec3 src, vec3 dst, float alpha){" \
    "return mix(src, dst, alpha);}"

static CGEConstString s_fshBorderS = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D borderTexture;)
#if defined(CGE_TEXTURE_PREMULTIPLIED) && CGE_TEXTURE_PREMULTIPLIED
    BLENDBORDER_PREMULTIPLIED
#else
	BLENDBORDER_NOT_PREMULTIPLIED
#endif
    CGE_SHADER_STRING(void main() {
	...
	BLENDBORDER_NOT_PREMULTIPLIED
        ...
        CGE_SHADER_STRING(void main() {
            ...
            if (textureCoordinate.y <= borderThickness.y)
            {
                ...
                {
                    coord = vec2(mod((textureCoordinate.x - borderThickness.x), borderLengthHV.x) / borderLengthHV.x,
                                 textureCoordinate.y / borderThickness.y) *
                            borderBlock[1].zw +
                        borderBlock[1].xy;
                ...
                {
                    coord = vec2((textureCoordinate.x - 1.0 + borderThickness.x) / borderThickness.x,
                                 textureCoordinate.y / borderThickness.y) *
                            borderBlock[2].zw +
                        borderBlock[2].xy;
            ...
            else if (textureCoordinate.y <= 1.0 - borderThickness.y)
            {
                ...
                {
                    coord = vec2((textureCoordinate.x - 1.0 + borderThickness.x) / borderThickness.x,
                                 mod(textureCoordinate.y - borderThickness.y, borderLengthHV.y) / borderLengthHV.y) *
                            borderBlock[3].zw +
                        borderBlock[3].xy;
                ...
                {
                    coord = vec2(textureCoordinate.x / borderThickness.x,
                                 mod(textureCoordinate.y - borderThickness.y, borderLengthHV.y) / borderLengthHV.y) *
                            borderBlock[7].zw +
                        borderBlock[7].xy;
            ...
            else if (textureCoordinate.y >= 1.0 - borderThickness.y)
            {
                ...
                {
                    coord = vec2(textureCoordinate.x / borderThickness.x,
                                 (textureCoordinate.y - 1.0 + borderThickness.y) / borderThickness.y) *
                            borderBlock[6].zw +
                        borderBlock[6].xy;
                ...
                {
                    coord = (textureCoordinate - 1.0 + borderThickness) / borderThickness * borderBlock[4].zw +
                        borderBlock[4].xy;
...
static CGEConstString s_fshCoverSKR =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D borderTexture; uniform float aspectRatio;)
#if defined(CGE_TEXTURE_PREMULTIPLIED) && CGE_TEXTURE_PREMULTIPLIED
        BLENDBORDER_PREMULTIPLIED
#else
	BLENDBORDER_NOT_PREMULTIPLIED
...
static CGEConstString s_fshCoverCorners = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D borderTexture;
    uniform vec2 borderThickness; uniform vec4 borderBlocks[4];  // leftTop, rightTop, leftBottom, rightBottom;
    )
#if defined(CGE_TEXTURE_PREMULTIPLIED) && CGE_TEXTURE_PREMULTIPLIED
    BLENDBORDER_PREMULTIPLIED
#else
    ...
    CGE_SHADER_STRING(
        ...
        void main() {
            ...
            else if (textureCoordinate.x >= 1.0 - borderThickness.x)
            {
                ...
                {
                    coord = (textureCoordinate - 1.0 + borderThickness) / borderThickness * borderBlocks[3].zw +
                        borderBlocks[3].xy;
                    vec4 tex = texture2D(borderTexture, coord);
                    src.rgb = blend(src.rgb, tex.rgb, tex.a);
                }
...
static CGEConstString s_fshCoverUDT =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  ...
                                  uniform float lengthH; uniform float borderThickness;)
#if defined(CGE_TEXTURE_PREMULTIPLIED) && CGE_TEXTURE_PREMULTIPLIED
        BLENDBORDER_PREMULTIPLIED
#else
	BLENDBORDER_NOT_PREMULTIPLIED
        ...
        CGE_SHADER_STRING(void main() {
            ...
            {
                coord = vec2(mod(textureCoordinate.x, lengthH) / lengthH,
                             (textureCoordinate.y - 1.0 + borderThickness) / borderThickness) *
                        borderBlocks[1].zw +
                    borderBlocks[1].xy;
                vec4 frameBottom = texture2D(borderTexture, coord);
                src.rgb = blend(src.rgb, frameBottom.rgb, frameBottom.a);
...
static CGEConstString s_fshCoverSWithTex = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D borderTexture;
    uniform vec4 borderBlocks[2]; uniform vec4 texArea;  // = vec4(0.0, 0.6, 0.45215, 0.21);
    )
#if defined(CGE_TEXTURE_PREMULTIPLIED) && CGE_TEXTURE_PREMULTIPLIED
    BLENDBORDER_PREMULTIPLIED
#else
...
static CGEConstString s_fshCoverSWithTexAndRatio =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D borderTexture; uniform vec4 borderBlocks[2]; uniform vec4 texArea;
...
static CGEConstString s_fshCoverMWithTex =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D borderTexture; uniform vec2 borderThickness;
                                  uniform vec2 borderLengthHV;
                                  // leftTop, Top, rightTop, right, rightBottom, bottom, leftBottom, left
                                  uniform vec4 borderBlock[8]; uniform vec4 texBlock; uniform vec4 texArea;)
#if defined(CGE_TEXTURE_PREMULTIPLIED) && CGE_TEXTURE_PREMULTIPLIED
        BLENDBORDER_PREMULTIPLIED
#else
	BLENDBORDER_NOT_PREMULTIPLIED
...
static CGEConstString s_fshCoverRTAndLBWithTex =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D borderTexture; uniform vec4 textCoord; uniform vec4 textArea;
                                  uniform vec4 texRTCoord; uniform vec4 texRTArea; uniform vec4 texLBCoord;
            ...
            void main() {
                ...
                else
                {
                        ...
                        textureCoordinate.y >= texRTArea.y && textureCoordinate.y <= texRTArea.y + texRTArea.w)
                    {
                        ...
                        if (coord.x < textCoord.x || coord.x > textCoord.x + textCoord.z || coord.y < textCoord.y ||
                            coord.y > textCoord.y + textCoord.w)
                        {
...
static CGEConstString s_fshCoverBottomKR = CGE_SHADER_STRING_PRECISION_M(
    ...
    CGE_SHADER_STRING(
        ...
        void main() {
            ...
            {
                vec2 coord = vec2(mod(textureCoordinate.x, texCoord.y) / texCoord.y,
                                  (textureCoordinate.y + texCoord.x - 1.0) / texCoord.x);
                vec4 tex = texture2D(borderTexture, coord);
...
static CGEConstString s_fshCoverUpDown =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D borderTexture; uniform float lengthH;
...
static CGEConstString s_fshCoverLeftRight =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D borderTexture; uniform float lengthV;
                                  uniform float borderThickness;)
#if defined(CGE_TEXTURE_PREMULTIPLIED) && CGE_TEXTURE_PREMULTIPLIED
        BLENDBORDER_PREMULTIPLIED
#else
	BLENDBORDER_NOT_PREMULTIPLIED
        ...
        CGE_SHADER_STRING(void main() {
            ...
            {
                coord = vec2((textureCoordinate.x - 1.0 + borderThickness) / borderThickness,
                             mod(textureCoordinate.y, lengthV) / lengthV);
                vec4 frameBottom = texture2D(borderTexture, coord);
                src.rgb = blend(src.rgb, frameBottom.rgb, frameBottom.a);
            }
...
void CGEBorderMFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...
void CGEBorderCoverCornersFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
...
void CGEBorderCoverUDTFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                             GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...

    {
        ...
        if (aspectRatio < 1.0f) thickness *= aspectRatio;
...
void CGEBorderSWithTexFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                             GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...
void CGEBorderMWithTexFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                             GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...

    {
        ...
        m_program.sendUniformf(paramTexAreaName, x, y, w, h);
        m_program.sendUniformf(paramThicknessName, thicknessX, thicknessY);
        m_program.sendUniformf(paramBorderLengthHVName, thicknessX * m_lengthHParam * m_texAspectRatio,
                               thicknessY * m_lengthVParam / m_texAspectRatio);
    }
    glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
    cgeCheckGLError("glDrawArrays");
}
...
void CGEBorderCoverRTAndLBWithTexFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                                        GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...
void CGEBorderCoverBottomKRFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                                  GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...
void CGEBorderCoverUpDownFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                                GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
...
void CGEBorderCoverLeftRightFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
                                                   GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
