...
public:
    ...
    // Call this function to retrieve the final result of the handler's current processing pass (the handler becomes
    // invalid after calling this and must be re-initialized)
    ...
    inline const GLfloat* getPosVertices() const { return CGEGlobalConfig::sVertexDataCommon; };
...
public:
    ...
    bool initWithRawBufferData(const void* data, GLint w, GLint h, CGEBufferFormat format,
                               bool bEnableReversion = true);
    ...
    // Apply a specific filter from the currently stored filter list
    // The last filter of all would be used if the index is -1
    bool processingWithFilter(GLint index);

    // Apply a filter that has not been added to the handler for standalone processing.
    ...
    // Helper methods
    bool copyTexture(GLuint dst, GLuint src);  // Uses the handler's internal image dimensions
