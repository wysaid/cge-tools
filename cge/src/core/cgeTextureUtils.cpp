...
static CGEConstString s_vsh = CGE_SHADER_STRING(attribute vec2 vPosition; varying vec2 texCoord; uniform mat2 rotation;
                                                uniform vec2 flipScale; void main() {
                                                    gl_Position = vec4(vPosition, 0.0, 1.0);
                                                    texCoord = flipScale * (vPosition / 2.0 * rotation) + 0.5;
                                                });

static CGEConstString s_fsh =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 texCoord; uniform sampler2D inputImageTexture; void main() {
        gl_FragColor = texture2D(inputImageTexture, texCoord);
        // gl_FragColor = vec4(texCoord, 0.0, 1.0);
    });

static CGEConstString s_vshMask =
    CGE_SHADER_STRING(attribute vec2 vPosition; varying vec2 texCoord; varying vec2 maskCoord;

                      uniform mat2 texRotation; uniform vec2 texFlipScale;

                      uniform mat2 maskRotation; uniform vec2 maskFlipScale;

                      void main() {
                          gl_Position = vec4(vPosition, 0.0, 1.0);
                          texCoord = texFlipScale * (vPosition / 2.0 * texRotation) + 0.5;
                          // If the mask size differs from the source, flipscale is needed to match them
                          maskCoord = maskFlipScale * (vPosition / 2.0 * maskRotation) + 0.5;
                      });


static CGEConstString s_fshMask =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 texCoord; varying vec2 maskCoord; uniform sampler2D inputImageTexture;
                                  uniform sampler2D maskTexture; void main() {
                                      gl_FragColor = texture2D(inputImageTexture, texCoord);

                                      // mask is typically single-channel

                                      // No pre-multiplication
...
static CGEConstString s_vshExternal_OES =
    CGE_SHADER_STRING(attribute vec2 vPosition; varying vec2 texCoord; uniform mat4 transform; uniform mat2 rotation;
                      uniform vec2 flipScale; void main() {
                          gl_Position = vec4(vPosition, 0.0, 1.0);
...
static CGEConstString s_fshExternal_OES =
    "#extension GL_OES_EGL_image_external : require\n" CGE_SHADER_STRING_PRECISION_M(
        varying vec2 texCoord; uniform samplerExternalOES inputImageTexture;
        void main() { gl_FragColor = texture2D(inputImageTexture, texCoord); });

#endif

CGEConstString s_fshYUVConvert = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 texCoord; uniform sampler2D luminanceTexture; uniform sampler2D chrominanceTexture; void main() {
        vec3 yuv;
        vec3 rgb;
        ...
        rgb = mat3(1.0, 1.0, 1.0, 0.0, -0.18732, 1.8556, 1.57481, -0.46813, 0.0) * yuv;
...
bool TextureDrawer::init()
{
    ...
    if (m_vertBuffer == 0) return false;
    glBindBuffer(GL_ARRAY_BUFFER, m_vertBuffer);
    glBufferData(GL_ARRAY_BUFFER, sizeof(CGEGlobalConfig::sVertexDataCommon), CGEGlobalConfig::sVertexDataCommon,
                 GL_STATIC_DRAW);
...
void TextureDrawer::_rotate(GLint location, float rad)
{
    ...
    float mat2[] = {cosRad, sinRad, -sinRad, cosRad};
...
{
    m_framebuffer.bind();
    if (fullSize) glViewport(0, 0, m_texSize.width, m_texSize.height);
    TextureDrawer::drawTexture(src);
}
...
bool TextureDrawerWithMask::init()
{
    ...
    if (m_rotLoc < 0 || m_flipScaleLoc < 0 || m_maskRotLoc < 0 || m_maskFlipScaleLoc < 0)
    {
        CGE_LOG_ERROR("TextureDrawer program init error...");
    }
    setRotation(0.0f);
...
{
    if (maskTexture == m_maskTexture) return;
...
{
    if (!TextureDrawer::init()) return false;
