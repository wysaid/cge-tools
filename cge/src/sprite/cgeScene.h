...
namespace CGE
{
	...
	// 
	// 	Note: SceneInterface uses the xOy plane as the navigation ground by default, with the positive z-axis pointing upward
	// 	SceneInterface only provides relevant matrices and vectors; it does not provide any rendering interface.
	...
	public:
		...
		inline void updateView()
		{
			...
			if(m_upDirRot != 0.0f) //No need for exact float-to-zero comparison; this just checks whether it has been set to 0
		...
		//Same as above; looking straight up is not allowed during scene navigation (tilting the head cannot form a flat angle)
