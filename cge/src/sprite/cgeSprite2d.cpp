...
//const vec4 viewArea = vec4(0.0,0.0,1.0,1.0); //Define the visible texture region; xy range [0, 1), zw range: (0, 1]
...
// 	}
	...
	Sprite2d::Sprite2d() { assert(0); } //Compatibility interface
...
//		glUniform1i(m_textureLocation, 0);
	...
	Sprite2dWithAlphaGradient::Sprite2dWithAlphaGradient() { assert(0); } //Compatibility interface
...
// 	}
    ...
    void Sprite2dInterChangeMultiple::jumpToFrame(int frameIndex)
    {
		...
		if(m_frameIndex >= m_vecFrames.size())
			m_frameIndex = m_vecFrames.size() - 1;
        _setToFrame(m_vecFrames[m_frameIndex]);
    }
