...
#include "../core/cgeGlobal.h"

#define LOG_FOR_LACKOF_FRAMES CGE_LOG_ERROR("Not enough frames!\n")

static CGEConstString s_vshSprite2d = CGE_SHADER_STRING(
    // Range: [-1, 1]
    attribute vec2 aPosition; varying vec2 vTextureCoord; uniform mat4 spriteModelViewProjection;
    uniform vec2 spriteHalfTexSize;

    uniform float rotation; uniform vec2 spriteScaling; uniform vec2 spriteTranslation; uniform vec2 spriteHotspot;
    uniform vec2 canvasflip; uniform vec2 spriteflip; uniform float zIndex;
    ...
    void main() {
        ...
        vec2 pos =
            mat2ZRotation(rotation) * (aPosition * spriteHalfTexSize - hotspot) * spriteScaling + spriteTranslation;
...
static CGEConstString s_vshSprite2dExt = CGE_SHADER_STRING(
    ...
    uniform vec2 scaleRatio;  // Scale ratio, used to implement tiling across the entire canvas.
...
static CGEConstString s_vshSprite2dInterChange = CGE_SHADER_STRING(
    ...
    // const vec4 viewArea = vec4(0.0,0.0,1.0,1.0); //Define the visible texture region; xy range [0, 1), zw range: (0,
    // 1]
    ...
    void main() {
        ...
        vec2 texSize = spriteHalfTexSize * viewArea.zw;  // Scale the entire image to the size of the view area
...
static CGEConstString s_vshSprite2dWith3dSpace = CGE_SHADER_STRING(
    ...
    uniform mat3 rotation; uniform vec2 spriteScaling; uniform vec2 spriteTranslation; uniform vec3 spriteHotspot;
    uniform vec2 canvasflip; uniform vec2 spriteflip; uniform float zIndex;
...
static CGEConstString s_vshBlank = CGE_SHADER_STRING(attribute vec4 aPosition;
...
static CGEConstString s_fshBlank =
    CGE_SHADER_STRING_PRECISION_L(void main() { gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0); });
...
{
    assert(m_refCount != nullptr && *m_refCount == 0 && m_pnt != nullptr);  // Unknown error, please investigate
...
SpriteInterface2d::SpriteInterface2d() :
    m_pos(0.0f, 0.0f), m_scaling(1.0f, 1.0f), m_hotspot(0.0f, 0.0f), m_rotation(0.0f), m_alpha(1.0f), m_zIndex(0.0f)
{}
...
Sprite2d* Sprite2d::sCreateWithBuffer(const void* bytes, int width, int height, CGEBufferFormat format)
{
    ...
    char* tmpBuffer =
        cgeGetScaledBufferInSize(bytes, width, height, channel, cgeGetMaxTextureSize(), cgeGetMaxTextureSize());
...
Sprite2d::Sprite2d(bool bInitProgram) : SpriteInterface2d()
{
    assert(!bInitProgram);
}
Sprite2d::Sprite2d(GLuint textureID, int width, int height, bool bInitProgram) :
    SpriteInterface2d(), m_texture(textureID, width, height)
{
    if (bInitProgram)
    {
        _initProgram();
...
Sprite2d::Sprite2d(const SharedTexture& texture, bool bInitProgram) : SpriteInterface2d(), m_texture(texture)
{
    if (bInitProgram)
    {
        _initProgram();
...
Sprite2d::Sprite2d(GLuint textureID, int width, int height) : SpriteInterface2d(), m_texture(textureID, width, height)
{
    _initProgram();
}
...
{
    return (Sprite2d::spVertexShader && Sprite2d::spFragmentShader) &&
        m_program.linkWithShaderObject(*Sprite2d::spVertexShader, *Sprite2d::spFragmentShader, false);
}
...
bool Sprite2d::_initProgram()
{
    ...
    if (!(Sprite2d::_tryLinkWithStaticShader() || m_program.initWithShaderStrings(s_vshSprite2d, s_fshSprite2d)))
    {
        _clearProgram();
...
void Sprite2d::_initProgramVars()
{
    ...
    //		glUniform1i(m_textureLocation, 0); Use uniform variable default value 0
...
Sprite2dExt::Sprite2dExt()
{
    assert(0);
}
Sprite2dExt::Sprite2dExt(GLuint textureID, int width, int height) :
    Sprite2d(textureID, width, height, false),
    m_lineSprite(nullptr),
    m_blendMode(CGEGLOBAL_BLEND_NONE),
    m_lineWidth(4.0f)
{
    _initProgram();
}
Sprite2dExt::Sprite2dExt(const SharedTexture& texture) :
    Sprite2d(texture, false), m_lineSprite(nullptr), m_blendMode(CGEGLOBAL_BLEND_NONE), m_lineWidth(4.0f)
{
    _initProgram();
}
Sprite2dExt::~Sprite2dExt()
...
{
    return (Sprite2dExt::spVertexShader && Sprite2dExt::spFragmentShader) &&
        m_program.linkWithShaderObject(*Sprite2dExt::spVertexShader, *Sprite2dExt::spFragmentShader, false);
}
...
bool Sprite2dExt::clipZone()
{
    if (m_zone.size() < 3)
    {
        CGE_LOG_CODE(if (!m_zone.empty()) CGE_LOG_ERROR("Invalid Zone Number: %d\n!", (int)m_zone.size()););
        return false;
    }
    ...
    if (m_dataCache.size() < sz * 2) m_dataCache.resize(sz * 2);
...
void Sprite2dExt::_drawFunc()
{
    ...
    if (m_blendMode != CGEGLOBAL_BLEND_NONE) cgeSetGlobalBlendMode(m_blendMode);

    glDrawArrays(GL_TRIANGLE_FAN, 0, 4);

    if (m_blendMode != CGEGLOBAL_BLEND_NONE) cgeSetGlobalBlendMode(CGEGLOBAL_BLEND_ALPHA);
...
Sprite2dWithAlphaGradient::Sprite2dWithAlphaGradient(GLuint textureID, int width, int height) :
    Sprite2d(textureID, width, height, false)
{
    _initProgram();
}
Sprite2dWithAlphaGradient::Sprite2dWithAlphaGradient(const SharedTexture& texture) : Sprite2d(texture, false)
{
    _initProgram();
}
Sprite2dWithAlphaGradient::~Sprite2dWithAlphaGradient()
...
{
    return (Sprite2dWithAlphaGradient::spVertexShader && Sprite2dWithAlphaGradient::spFragmentShader) &&
        m_program.linkWithShaderObject(*Sprite2dWithAlphaGradient::spVertexShader,
                                       *Sprite2dWithAlphaGradient::spFragmentShader, false);
}
...
Sprite2dInterChange::Sprite2dInterChange(GLuint textureID, int width, int height) :
    Sprite2d(textureID, width, height, false)
{
    _initProgram();
}
Sprite2dInterChange::Sprite2dInterChange(const SharedTexture& texture) : Sprite2d(texture, false)
{
    _initProgram();
}
Sprite2dInterChange::~Sprite2dInterChange()
...
bool Sprite2dInterChange::_initProgram()
{
    ...
    glUniform4f(m_viewAreaLocation, 0.0f, 0.0f, 1.0f, 1.0f);  // Use full resolution by default.
...
{
    return (Sprite2dInterChange::spVertexShader && Sprite2dInterChange::spFragmentShader) &&
        m_program.linkWithShaderObject(*Sprite2dInterChange::spVertexShader, *Sprite2dInterChange::spFragmentShader,
                                       false);
}
...
Sprite2dInterChangeMultiple::Sprite2dInterChangeMultiple(int width, int height) :
    Sprite2dInterChange(SharedTexture(width, height))
{}
...
void Sprite2dInterChangeMultiple::jumpToFrame(int frameIndex)
{
    ...
    if (m_frameIndex >= m_vecFrames.size()) m_frameIndex = m_vecFrames.size() - 1;
    _setToFrame(m_vecFrames[m_frameIndex]);
}

void Sprite2dInterChangeMultiple::jumpToLastFrame()
{
    if (!m_vecFrames.empty()) m_frameIndex = (GLuint)m_vecFrames.size() - 1;
}
...
void Sprite2dInterChangeMultiple::_calcFrames()
{
    ...
    for (auto& tex : m_vecTextures)
    {
        ...
        if (tex.count < total) total = tex.count;
...
{
    if (!m_frameTextures.empty()) glDeleteTextures((int)m_frameTextures.size(), m_frameTextures.data());
...
Sprite2dWith3dSpace::Sprite2dWith3dSpace(GLuint textureID, int width, int height) :
    Sprite2d(textureID, width, height, false)
{
    _initProgram();
}
...
{
    return (Sprite2dWith3dSpace::spVertexShader && Sprite2dWith3dSpace::spFragmentShader) &&
        m_program.linkWithShaderObject(*Sprite2dWith3dSpace::spVertexShader, *Sprite2dWith3dSpace::spFragmentShader,
                                       false);
}
