...
#define INIT_SPRITE_STATIC_SHADERS(SpriteType)                                                            \
    do                                                                                                    \
    {                                                                                                     \
        if (SpriteType::spVertexShader == nullptr) SpriteType::spVertexShader = new ShaderObject();       \
        if (SpriteType::spFragmentShader == nullptr) SpriteType::spFragmentShader = new ShaderObject();   \
                                                                                                          \
        ...
        {                                                                                                 \
            CGE_LOG_ERROR("Failed to start " #SpriteType " optimization");                                \
...
// TODO: This approach cannot handle large corner angles; needs a different solution
static CGEConstString s_vshGeometryLineStrip2d = CGE_SHADER_STRING(
    attribute vec2 aPosition;  // Uses absolute coordinates
    attribute vec3 aLineData;

    varying float vGradient;

    uniform vec2 lineWidth; uniform vec2 lineFlip;

    uniform float gradient;

    vec2 rotate90(vec2 v) { return vec2(v.y, -v.x); }
...
CGE_LOG_CODE(static bool sRemoveMe(std::vector<CGE::SpriteCommonSettings*> & vec, CGE::SpriteCommonSettings * sprite) {
    ...
    return false;
             ...
             // Store all existing sprites, detect memory leaks, or perform some global settings.
...
CGE_LOG_CODE(std::vector<SpriteCommonSettings*>& SpriteCommonSettings::getDebugManager() { return s_spriteManager; })

SpriteCommonSettings::SpriteCommonSettings()
{
    CGE_LOG_CODE(s_spriteManager.push_back(this);)
}

SpriteCommonSettings::~SpriteCommonSettings()
{
    CGE_LOG_CODE(if (!sRemoveMe(s_spriteManager, this))
...
GeometryLineStrip2d::GeometryLineStrip2d() : m_color(1.0f, 1.0f, 1.0f, 1.0f), m_lineWidth(4.0f), m_gradient(1.0f)
{
    ...
    if (s_program == nullptr) _initProgram();
}
...
void GeometryLineStrip2d::_setUniforms()
{
    ...
    glUniform2f(s_lineWidthLocation, m_lineWidth / CGEGlobalConfig::viewWidth,
                m_lineWidth / CGEGlobalConfig::viewHeight);
...
{
    if (m_points.size() < 2)  // Insufficient points, cannot draw
        return;
