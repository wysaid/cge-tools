...
namespace CGE
{
	...
	{		
		...
		{
			return false; // Cannot find video stream
		}

		if(m_context->audioStreamIndex == -1)
		{
			CGE_LOG_INFO("No audio stream found, video will be muted...\n");
		}
		...
		{
			return false; // Video decode failed
		}
		...
		if(m_context->audioStreamIndex != -1)
		{
			...
			{
				CGE_LOG_ERROR("Audio decode failed! Proceeding without audio...\n");
				m_context->audioStreamIndex = -1;
	...
	CGEFrameTypeNext CGEVideoDecodeHandler::queryNextFrame()
	{
		...
		{  
			...
			{  
				...
				if(gotFrame)
				{
// 					if(m_context->packet.dts != AV_NOPTS_VALUE)
//                     {
//                         m_context->pVideoFrame->pts = av_frame_get_best_effort_timestamp(m_context->pVideoFrame);
//                         double tmp = av_q2d(m_context->pVideoStream->codec->time_base);
//                         m_context->pVideoFrame->pts = m_context->pVideoFrame->pts * tmp;
//                     }
//                     else
//                     {
...
//                     AVRational frameRate = av_guess_frame_rate(m_context->pFormatCtx, m_context->pVideoStream, m_context->pVideoFrame);
// 
//                     m_currentTimestamp = m_context->pVideoFrame->pts * 1000.0 / av_q2d(frameRate);

                    m_currentTimestamp = 1000.0 * (m_context->packet.dts - m_context->pVideoStream->start_time) * av_q2d(m_context->pVideoStream->time_base);
