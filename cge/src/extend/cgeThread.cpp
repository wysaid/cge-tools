...
bool CGEThreadPool::isActive()
{
    ...
    if (!m_workList.empty()) return true;
...
void CGEThreadPool::Worker::_run()
{
    for (;;)
    {
...

        {
            ...
            if (m_pool.m_threadOver || m_shouldLeave) break;

            if (m_pool.m_workList.empty())
            {
                m_runningStatus = false;

                if (m_pool.m_threadJoining) break;

                m_pool.m_condition.wait(lock);

                if (m_pool.m_threadOver || m_shouldLeave) return;
            }

            if (m_pool.m_workList.empty()) continue;
