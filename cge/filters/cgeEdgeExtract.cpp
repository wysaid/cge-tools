...
static CGEConstString s_fshFilter =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 sft;
...
static CGEConstString s_fshDownOdd =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 sft;
...
static CGEConstString s_fshDownEvn =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 sft;
...
static CGEConstString s_fshResize = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 fac;


    void main() { gl_FragColor = texture2D(inputImageTexture, textureCoordinate * fac); });


static CGEConstString s_fshEdge = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 sft; uniform float evn;
    const vec4 weight = vec4(0.299, 0.587, 0.114, 0.0);


    const mat4 m1 = mat4(-1.0 / 6.0, 0.0, 1.0 / 6.0, -1.0 / 6.0, -1.0 / 6.0, -1.0 / 6.0, -1.0 / 6.0, 0.0,

                         0.1, -0.2, 0.1, 0.3, 0.1, 0.3, 0.1, -0.2);


    const mat4 m2 = mat4(1.0 / 6.0, -1.0 / 6.0, 0.0, 1.0 / 6.0, 0.0, 1.0 / 6.0, 1.0 / 6.0, 1.0 / 6.0,

                         0.3, 0.1, -0.2, 0.1, -0.2, 0.1, 0.3, 0.1);
...
static CGEConstString s_fshReg =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  ...
                                  void main() {
                                      // vec3 p1 = extract(texture2D(inputImageTexture, textureCoordinate + sft *
                                      // vec2(-1.0 + evn,-1.0 - evn)));

                                      // vec3 p2 = extract(texture2D(inputImageTexture, textureCoordinate + sft * vec2(
                                      // 0.0 + evn,-1.0      )));

                                      // vec3 p3 = extract(texture2D(inputImageTexture, textureCoordinate + sft *
                                      // vec2( 1.0 + evn,-1.0 + evn)));

                                      // vec3 p4 = extract(texture2D(inputImageTexture, textureCoordinate + sft *
                                      // vec2(-1.0      , 0.0 - evn)));

                                      // vec3 p5 = extract(texture2D(inputImageTexture, textureCoordinate + sft * vec2(
                                      // 0.0      , 0.0      )));

                                      // vec3 p6 = extract(texture2D(inputImageTexture, textureCoordinate + sft *
                                      // vec2( 1.0      , 0.0 + evn)));

                                      // vec3 p7 = extract(texture2D(inputImageTexture, textureCoordinate + sft *
                                      // vec2(-1.0 - evn, 1.0 - evn)));

                                      // vec3 p8 = extract(texture2D(inputImageTexture, textureCoordinate + sft * vec2(
                                      // 0.0 - evn, 1.0      )));

                                      // vec3 p9 = extract(texture2D(inputImageTexture, textureCoordinate + sft *
                                      // vec2( 1.0 - evn, 1.0 + evn)));


                                      // vec3 abc2 = (p1 * p1 + p3 * p3 + p7  * p7 + p9 * p9) * vec3(0.6) + p2 * p2 + p4
                                      // * p4 + p6  * p6 + p8 * p8 + p5 * p5;

                                      // vec3 abcx = (p1 * p1.yzx + p3 * p3.yzx + p7  * p7.yzx + p9 * p9.yzx) *
                                      // vec3(0.6) + p2.yzx * p2 + p4 * p4.yzx + p6  * p6.yzx + p8 * p8.yzx + p5 *
                                      // p5.yzx;
...
static CGEConstString s_fshUpOdd = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 isft; uniform vec2 sft;
...
static CGEConstString s_fshUpCompressOdd = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D inputImageTextureS;
    uniform vec2 isft; uniform vec2 sft;
...
private:
    ...
    void size_downOdd(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID);

    void size_downEvn(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID);

    void size_upOdd(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID);

    void size_upEvn(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID);

    void size_upCompressOdd(GLuint srcTexture, GLuint cpTexture, GLuint dstTexture, CGESizei dstSize,
                            GLuint vertexBufferID);

    void size_upCompressEvn(GLuint srcTexture, GLuint cpTexture, GLuint dstTexture, CGESizei dstSize,
                            GLuint vertexBufferID);


    void draw_tmp_tex(GLuint srcTexture, GLuint vertexBufferID);

    void draw_tex(GLuint srcTexture, GLuint disTexture, CGESizei disSize, float wf, float hf, GLuint vertexBufferID);

    void draw_edge(int idx, bool evn, GLuint vertexBufferID);

    void draw_reg(int idx, bool evn, GLuint vertexBufferID);

    void make_small_map(CGESizei sz, GLuint srcTexture, GLuint vertexBufferID);

    void start_shader(CGE::ProgramObject& shader, GLuint vertexBufferID)
    {
...
CGEEdgeExtract::CGEEdgeExtract() : m_size(0, 0), m_sat(1.0), m_emin(0.0), m_emax(1.0), m_level(-1)
{}
...
bool CGEEdgeExtract::init()
{
    ...
    if (!initShader(m_filterShader, s_fshFilter)) return false;

    if (!initShader(m_downOddShader, s_fshDownOdd)) return false;

    if (!initShader(m_downEvnShader, s_fshDownEvn)) return false;

    if (!initShader(m_upOddShader, s_fshUpOdd)) return false;

    if (!initShader(m_upEvnShader, s_fshUpEvn)) return false;

    if (!initShader(m_upCompressOddShader, s_fshUpCompressOdd)) return false;

    if (!initShader(m_upCompressEvnShader, s_fshUpCompressEvn)) return false;

    if (!initShader(m_resizeShader, s_fshResize)) return false;

    if (!initShader(m_edgeShader, s_fshEdge)) return false;

    if (!initShader(m_regShader, s_fshReg)) return false;
...
void CGEEdgeExtract::size_downOdd(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_downOddShader, vertexBufferID);
...
void CGEEdgeExtract::size_downEvn(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_downEvnShader, vertexBufferID);
...
void CGEEdgeExtract::size_upOdd(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_upOddShader, vertexBufferID);
...
void CGEEdgeExtract::size_upEvn(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_upEvnShader, vertexBufferID);
...
void CGEEdgeExtract::size_upCompressOdd(GLuint srcTexture, GLuint cpTexture, GLuint dstTexture, CGESizei dstSize,
                                        GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_upCompressOddShader, vertexBufferID);
...
void CGEEdgeExtract::size_upCompressEvn(GLuint srcTexture, GLuint cpTexture, GLuint dstTexture, CGESizei dstSize,
                                        GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_upCompressEvnShader, vertexBufferID);
...
void CGEEdgeExtract::draw_tex(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, float wf, float hf,
                              GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_resizeShader, vertexBufferID);
...
void CGEEdgeExtract::draw_edge(int idx, bool evn, GLuint vertexBufferID)
{
    CGESizei sz = m_texSizes[idx];
    glViewport(0, 0, sz.width, sz.height);

    start_shader(m_edgeShader, vertexBufferID);
    ...
    // draw_reg(idx,evn,vertexBufferID);
}

void CGEEdgeExtract::draw_reg(int idx, bool evn, GLuint vertexBufferID)
{
    CGESizei sz = m_texSizes[idx];
    glViewport(0, 0, sz.width, sz.height);

    start_shader(m_regShader, vertexBufferID);
...
{
    return cgeGenTextureWithBuffer(nullptr, size.width, size.height, GL_RGBA, GL_UNSIGNED_BYTE, 4, 0,
                                   nearest ? GL_NEAREST : GL_LINEAR);
}

void CGEEdgeExtract::draw_tmp_tex(GLuint srcTexture, GLuint vertexBufferID)
{

    int idx = (int)m_texSizes.size() - 1;
    draw_tex(srcTexture, m_tmpTexs[idx][1], m_texSizes[idx], m_wFac, m_hFac, vertexBufferID);

    {
        glViewport(0, 0, m_texSizes[idx].width, m_texSizes[idx].height);

        start_shader(m_filterShader, vertexBufferID);
    ...
    draw_edge(idx, !(idx % 2), vertexBufferID);

    --idx;

    size_downOdd(m_tmpTexs[idx + 1][0], m_tmpTexs[idx][0], m_texSizes[idx], vertexBufferID);
    draw_edge(idx, !(idx % 2), vertexBufferID);
    ...
    for (; idx >= 1; --idx)
    {
        size_downEvn(m_tmpTexs[idx + 1][0], m_tmpTexs[idx][0], m_texSizes[idx], vertexBufferID);
        draw_edge(idx, !(idx % 2), vertexBufferID);
    ...
    size_upOdd(m_tmpTexs[i - 1][si], m_tmpTexs[i][di], m_texSizes[i], vertexBufferID);
    ...
    for (; i < m_texSizes.size(); ++i)
    {
        if (i % 2)
        {
            size_upCompressOdd(m_tmpTexs[i - 1][di], m_tmpTexs[i][si], m_tmpTexs[i][di], m_texSizes[i], vertexBufferID);
        }
        else
        {
            size_upCompressEvn(m_tmpTexs[i - 1][di], m_tmpTexs[i][si], m_tmpTexs[i][di], m_texSizes[i], vertexBufferID);
        }
    }
...
void CGEEdgeExtract::make_small_map(CGESizei sz, GLuint srcTexture, GLuint vertexBufferID)
{

    // if(flush_flag || sz.width != m_size.width || sz.height != m_size.height)
    //{
    init_tmp_tex(sz);
    draw_tmp_tex(srcTexture, vertexBufferID);
    //	flush_flag = false;
    //}
}

void CGEEdgeExtract::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{

    handler->setAsTarget();

    make_small_map(handler->getOutputFBOSize(), srcTexture, vertexBufferID);
    ...
    if (i < m_texSizes.size())
    {
        ...
        {
            size_upEvn(m_tmpTexs[i - 1][si], m_tmpTexs[i][di], m_texSizes[i], vertexBufferID);
        }
        ...
        for (; i < m_texSizes.size(); ++i)
        {
            if (i % 2)
            {
                size_upOdd(m_tmpTexs[i - 1][di], m_tmpTexs[i][di], m_texSizes[i], vertexBufferID);
            }
            else
            {
                size_upEvn(m_tmpTexs[i - 1][di], m_tmpTexs[i][di], m_texSizes[i], vertexBufferID);
            }
        }
        sourcIdx = di;
    }
    ...
    draw_tex(m_tmpTexs.back()[sourcIdx], handler->getTargetTextureID(), handler->getOutputFBOSize(), 1.0 / m_wFac,
             1.0 / m_hFac, vertexBufferID);

    // draw_tex(m_tmpTexs[level + 1][0],handler->getTargetTextureID(),handler->getOutputFBOSize(),1.0 / m_wFac,1.0 /
    // m_hFac,vertexBufferID);
