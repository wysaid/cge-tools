...
public:
    typedef struct CurvePoint
    {
        ...
        bool operator<(const CurvePoint& cp) const { return x < cp.x; }
    } CurvePoint;
    ...
    static bool mergeCurveConst(std::vector<float>& dst, const std::vector<float>& late,
    ...
    static bool genCurve(std::vector<CurveData>& vec, const CurvePoint* pntsR, size_t cntR, const CurvePoint* pntsG,
                         size_t cntG, const CurvePoint* pntsB, size_t cntB);

    static bool genCurve(std::vector<CurveData>& vec, const CurvePoint* pnts, size_t cnt, size_t channel);

    static bool mergeCurveConst(std::vector<CurveData>& dst, const std::vector<CurveData>& late,
    ...
    static bool mergeCurve(std::vector<CurveData>& dst, std::vector<float>& late, std::vector<CurveData>& early,
                           unsigned channel);
    static void scaleCurve(std::vector<CurveData>& vec, size_t precision);

    // curveRGB's length is 3 times as cnt, ordered by r-g-b
    static bool loadCurve(std::vector<CurveData>& vec, const float* curve, size_t cnt, size_t dstChannel,
    ...
    virtual void setPoints(const CurvePoint* r, size_t nr, const CurvePoint* g, size_t ng, const CurvePoint* b,
                           size_t nb);
    ...
    virtual void loadCurves(const float* curveR, size_t nR, const float* curveG, size_t nG, const float* curveB,
                            size_t nB);
...
public:
    ...
    void setPoints(const CurvePoint* r, size_t nr, const CurvePoint* g, size_t ng, const CurvePoint* b, size_t nb);
    ...
    void loadCurves(const float* curveR, size_t nR, const float* curveG, size_t nG, const float* curveB, size_t nB);
...
public:
    ...
    void pushPoints(const CurvePoint* r, size_t nr, const CurvePoint* g, size_t ng, const CurvePoint* b, size_t nb);
    ...
    void pushCurves(const float* curveR, size_t nR, const float* curveG, size_t nG, const float* curveB, size_t nB);
...
// For structural clarity, some code is intentionally similar to CGECurveTexFilter
...
public:
    ...
    void setSamplerID(GLuint texID) { m_curveTexture = texID; }
