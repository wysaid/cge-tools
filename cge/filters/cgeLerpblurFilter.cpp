...
static CGEConstString s_fshScale =
...
{
    m_intensity = value;
    if (m_intensity > MAX_LERP_BLUR_INTENSITY) m_intensity = MAX_LERP_BLUR_INTENSITY;
}
...
void CGELerpblurFilter::setIntensity(float value)
{
    if (value <= 0.5f)
    {
        ...
        if (m_mipmapBase != 1.0f) setMipmapBase(1.0f);
    }
    else
    {
...
void CGELerpblurFilter::_genMipmaps(int width, int height)
{
    ...
    if (textureType == GL_FALSE)
    {
        ...
        // Apple A9 (iPhone 6s+) GL_UNSIGNED_BYTE texture precision is low; GL_HALF_FLOAT_OES must be used
        // For better compatibility, use GL_HALF_FLOAT_OES on all devices that support the EXT_color_buffer_half_float
        // extension
...
void CGELerpblurFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    ...
    if (m_texCache[0].texID == 0 || m_cacheTargetSize.width != sz.width || m_cacheTargetSize.height != sz.height ||
        m_isBaseChanged)
    {
...
{
    m_mipmapBase = value;
    if (m_mipmapBase < 0.6f) m_mipmapBase = 0.6f;
    m_isBaseChanged = true;
}
