...
const static char* const s_fshEdge = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform float stride; uniform float intensity; uniform vec2 norm;
...
CGEConstString s_vshEdgeSobel =
    CGE_SHADER_STRING(attribute vec2 vPosition; varying vec2 textureCoordinate; varying vec2 coords[8];

                      uniform vec2 samplerSteps; uniform float stride;
...
const static char* const s_fshEdgeSobel = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 samplerSteps;
    uniform float stride; uniform float intensity;
