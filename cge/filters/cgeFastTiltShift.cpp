...
static CGEConstString s_fshB1_1 =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 sft;


                                  void main() {
                                      vec4 src =
                                          texture2D(inputImageTexture, textureCoordinate + sft * vec2(-2.0, -1.0));
                                      vec4 dst = src * src * src;
                                      src = texture2D(inputImageTexture, textureCoordinate + sft * vec2(-2.0, 1.0));
                                      dst += src * src * src;
                                      src = texture2D(inputImageTexture, textureCoordinate + sft * vec2(2.0, -1.0));
...
static CGEConstString s_fshB2_P1 = CGE_SHADER_STRING(
    ...
    src = texture2D(inputImageTexture, textureCoordinate + sft * vec2(-3.0, -3.0)); dst += src * src * src;
    src = texture2D(inputImageTexture, textureCoordinate + sft * vec2(2.0, -3.0)); dst += src * src * src;
    src = texture2D(inputImageTexture, textureCoordinate + sft * vec2(-3.0, 2.0)); dst += src * src * src;
    src = texture2D(inputImageTexture, textureCoordinate + sft * vec2(2.0, 2.0)); dst += src * src * src;

    dst *= vec4(0.125);

    src = texture2D(inputImageTextureBlur, textureCoordinate + sft * vec2(0.0, -3.0)); dst += src * src * src;
    src = texture2D(inputImageTextureBlur, textureCoordinate + sft * vec2(-2.0, -1.0)); dst += src * src * src;
    src = texture2D(inputImageTextureBlur, textureCoordinate + sft * vec2(2.0, -1.0)); dst += src * src * src;
    src = texture2D(inputImageTextureBlur, textureCoordinate + sft * vec2(-2.0, 1.0)); dst += src * src * src;
    src = texture2D(inputImageTextureBlur, textureCoordinate + sft * vec2(2.0, 1.0)); dst += src * src * src;
    src = texture2D(inputImageTextureBlur, textureCoordinate + sft * vec2(0.0, 3.0)); dst += src * src * src;
...
static CGEConstString s_fshB2_P2 = CGE_SHADER_STRING(
    ...
    vec4 bs = vec4(1.0 / 4.0); vec4 p1 = vec4(-1.0, -1.0, 0.0, 0.0);
    ...
    dst += src * src * src; src = texture2D(inputImageTexture, textureCoordinate + sft * p1.zy); dst += src * src * src;
    src = texture2D(inputImageTexture, textureCoordinate + sft * p1.xw); dst += src * src * src;
    src = texture2D(inputImageTexture, textureCoordinate + sft * p1.zw); dst += src * src * src;

    dst *= bs;

);

static CGEConstString s_fshB2 =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  uniform sampler2D inputImageTextureBlur; uniform vec2 sft;
...
static CGEConstString s_fshB2_1 = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D inputImageTextureBlur;
    uniform sampler2D inputImageTextureUp; uniform vec2 sft; uniform vec2 upsft; uniform mat2 trans; uniform vec2 pos;
    uniform float th;
...
static CGEConstString s_fshB2_a = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D inputImageTextureBlur;
    uniform vec2 sft; uniform mat2 trans; uniform vec2 pos; uniform float th;
...
static CGEConstString s_fshB3 = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D inputImageTextureBlur;
    uniform sampler2D inputImageTextureBlur2; uniform vec2 sft;
...
static CGEConstString s_fshUp = CGE_SHADER_STRING_PRECISION_H(
    ...
    uniform sampler2D inputImageTextureS2; uniform vec4 m1; uniform vec4 m2; uniform mat2 trans; uniform vec2 pos;
    uniform float th; uniform float bsth;
...
private:
    ...
    void size_down(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID);
    ...
    void blur_tex(CGE::ProgramObject& shader, GLuint srcTexture, GLuint disTexture, CGESizei size,
                  GLuint vertexBufferID);

    void make_small_map(CGESizei sz, GLuint srcTexture, GLuint vertexBufferID);

    void draw_level_start(int level, GLuint vertexBufferID);

    void draw_level(int level, bool sf, float fac, GLuint vertexBufferID);
...
{
    if (!initShader(m_upShader, s_fshUp)) return false;

    if (!initShader(m_downShader, s_fshDown)) return false;

    if (!initShader(m_resizeShader, s_fshResize)) return false;

    if (!initShader(m_b1Shader, s_fshB1)) return false;

    if (!initShader(m_b1_1Shader, s_fshB1_1)) return false;

    if (!initShader(m_b2Shader, s_fshB2, s_fshB2_P1)) return false;

    if (!initShader(m_b2_1Shader, s_fshB2_1, s_fshB2_P1)) return false;

    if (!initShader(m_b2_1aShader, s_fshB2_1, s_fshB2_P2)) return false;

    if (!initShader(m_b2_aShader, s_fshB2_a, s_fshB2_P2)) return false;

    if (!initShader(m_b3Shader, s_fshB3)) return false;

    return true;
}


void CGEFastTiltShift::size_down(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_downShader, 0);
...
void CGEFastTiltShift::draw_tex(GLuint srcTexture, GLuint dstTexture, CGESizei dstSize, float wf, float hf,
                                GLuint vertexBufferID)
{
    glViewport(0, 0, dstSize.width, dstSize.height);

    start_shader(m_resizeShader, 0);
...
void CGEFastTiltShift::draw_tmp_tex(GLuint srcTexture, GLuint vertexBufferID)
{
    ...
    draw_tex(srcTexture, m_tmpTexs[idx][0], m_texSizes[idx], m_wFac[idx % 2], m_hFac[idx % 2], 0);
    blur_tex(m_b1Shader, m_tmpTexs[idx][0], m_tmpTexs[idx][1], m_texSizes[idx], 0);
    ...
    draw_tex(m_tmpTexs[idx + 1][0], m_tmpTexs[idx][0], m_texSizes[idx], m_wFac[idx % 2] / m_wFac[(idx + 1) % 2],
             m_hFac[idx % 2] / m_hFac[(idx + 1) % 2], 0);
    blur_tex(m_b1Shader, m_tmpTexs[idx][0], m_tmpTexs[idx][1], m_texSizes[idx], 0);
    glBindTexture(GL_TEXTURE_2D, m_tmpTexs[idx + 1][0]);
    ...
    for (; idx >= 2; --idx)
    {
        ...
        m_tmpTexs[idx].push_back(create_tex(m_texSizes[idx]));
        m_tmpTexs[idx].push_back(create_tex(m_texSizes[idx], false));
        size_down(m_tmpTexs[idx + 2][0], m_tmpTexs[idx][0], m_texSizes[idx], 0);
        blur_tex(m_b1Shader, m_tmpTexs[idx][0], m_tmpTexs[idx][1], m_texSizes[idx], 0);
    }
    ...
    m_tmpTexs[idx].push_back(create_tex(m_texSizes[idx]));
    m_tmpTexs[idx].push_back(create_tex(m_texSizes[idx]));
    blur_tex(m_b1_1Shader, m_tmpTexs[idx + 2][1], m_tmpTexs[idx][0], m_texSizes[idx], 0);
    draw_level_start(idx, 0);
...
inline void init_tex(std::vector<GLuint>& tex, CGESizei size)
{
    ...
    {
        GLuint texID =
            cgeGenTextureWithBuffer(nullptr, size.width, size.height, GL_RGBA, GL_UNSIGNED_BYTE, 4, 0, GL_NEAREST);
        tex.push_back(texID);
    }
}

void CGEFastTiltShift::blur_tex(CGE::ProgramObject& shader, GLuint srcTexture, GLuint dstTexture, CGESizei size,
                                GLuint vertexBufferID)
{
...
inline CGESizei cal_size(CGESizei os, float radius)
{
    ...
    if (os.width > os.height)
        return CGESizei(ls, os.height * ls / os.width);
    else
        return CGESizei(ls * os.width / os.height, ls);
}
...
void CGEFastTiltShift::draw_level_start(int level, GLuint vertexBufferID)
{
    if (level >= 2) return;
...
void CGEFastTiltShift::draw_level(int level, bool sf, float fac, GLuint vertexBufferID)
{
    if (level < 2) return;
...
void CGEFastTiltShift::make_small_map(CGESizei sz, GLuint srcTexture, GLuint vertexBufferID)
{

    if (flush_flag || sz.width != m_size.width || sz.height != m_size.height)
    {
        init_tmp_tex(sz);
        draw_tmp_tex(srcTexture, 0);
        flush_flag = false;
    }
    ...
    draw_level(level, true, pow(0.5f, (level - baseLevel - 1.0f) * 0.5f), 0);
    ++level;
    draw_level(level, true, pow(0.5f, (level - baseLevel - 1.0f) * 0.5f), 0);
    ++level;
    ...
    {
        draw_level(level, false, pow(0.5f, (level - baseLevel - 1.0f) * 0.5f), 0);
    }
...
void CGEFastTiltShift::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{

    handler->setAsTarget();

    make_small_map(handler->getOutputFBOSize(), srcTexture, 0);
    ...
    m_upShader.sendUniformf("m1", 1.0f / m_wFac[idx1 & 1], 1.0f / m_hFac[idx1 & 1], 0.5f / sz1.width,
                            0.5f / sz1.height);

    m_upShader.sendUniformf("m2", 1.0f / m_wFac[idx2 & 1], 1.0f / m_hFac[idx2 & 1], 0.5f / sz2.width,
                            0.5f / sz2.height);
