...
static CGEConstString s_fshE16 =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;

                                  const float fh_red = 0.1; const float fh_yellow = -0.7; const float fh_green = -0.7;
                                  const float fh_cyan = -0.7; const float fh_blue = -0.7; const float fh_magenta = 0.1;
...
static CGEConstString s_fshE21 = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 vSteps;
...
static CGEConstString s_fshE28 = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform vec2 colorScale; uniform vec2 vignette; const vec2 vignetteCenter = vec2(0.5, 0.5);
...
static CGEConstString s_fshE29 = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 colorScale;
...
static CGEConstString s_fshE39 = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D SamplerBackTex1;
    uniform float saturation; uniform vec2 colorScale; void main(void) {
        vec4 src;
        vec3 dst;
        vec3 tmp_s;
        float alpha;
...
static CGEConstString s_fshE87 =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  const vec3 vr = vec3(1.0, 0.835, 0.835); const vec3 vg = vec3(0.0, 0.588, 1.0);
...
static CGEConstString s_fshE88 = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform vec2 vSteps; const float intensity = 0.45;

    float getLum(vec3 src) { return dot(src, vec3(0.299, 0.587, 0.114)); }
...
static CGEConstString s_fshE90 = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec3 colorGradient[5];
...
static CGEConstString s_fshE91 = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 vignette;

    const vec3 c1 = vec3(0.992, 0.137, 0.314); const vec3 c2 = vec3(0.204, 0.98, 0.725);
    const vec2 vignetteCenter = vec2(0.5, 0.5);
...
static CGEConstString s_fshE99 =
    CGE_SHADER_STRING_PRECISION_H(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
...
static CGEConstString s_fshE118 = CGE_SHADER_STRING_PRECISION_H(
    ...
    const vec2 vignetteCenter = vec2(0.5, 0.5); const float vignetteStart = 0.0; const float vignetteRange = 0.8;
    ...
    void main(void) {
        ...
        src.rgb = vec3((lum - colorScale.x) / colorScale.y * alpha) *
            mat3(0.393, 0.769, 0.189, 0.349, 0.686, 0.168, 0.272, 0.534, 0.131);
...
static CGEConstString s_fshE73 = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D SamplerBackTex1;
    uniform vec3 curveArray1[256]; uniform vec3 curveArray2[256];
    ...
    void main(void) {
        ...
        vec3 dst_a = vec3(curveArray1[int(src.r * curvePrecision)].r, curveArray1[int(src.g * curvePrecision)].g,
                          curveArray1[int(src.b * curvePrecision)].b);

        vec3 dst_b = vec3(curveArray2[int(src.r * curvePrecision)].r, curveArray2[int(src.g * curvePrecision)].g,
...
static CGEConstString s_fshE74 = CGE_SHADER_STRING_PRECISION_M(
    ...
    void main(void) {
        ...
        vec3 dst_a = vec3(curveArray1[int(tmp.r * curvePrecision)].r, curveArray1[int(tmp.g * curvePrecision)].g,
                          curveArray1[int(tmp.b * curvePrecision)].b);

        vec3 dst_b = vec3(curveArray2[int(tmp.r * curvePrecision)].r, curveArray2[int(tmp.g * curvePrecision)].g,
...
static CGEConstString s_fshE76 = CGE_SHADER_STRING_PRECISION_M(
    ...
    vec3 prcSaturate50(vec3 src) {
        return src * mat3(0.650, 0.2935, 0.057, 0.1495, 0.7935, 0.057, 0.1495, 0.2935, 0.557);
    }
...
const GLfloat CGESpecialFilterE90::paramColorGradientValue[] = {0.0f,     0.0f,     32768.0f, 8000.0f,  7000.0f,
                                                                24576.0f, 16000.0f, 14000.0f, 16384.0f, 24000.0f,
                                                                21000.0f, 8192.0f,  32000.0f, 28000.0f, 0.0f};
...
bool CGESpecialFilterE90::init()
{
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE90))
    {
        ...
        if (index < 0)
        {
            ...
            glUniform3fv(index, 5, paramColorGradientValue);
        return true;
    }
    return false;
}

const GLfloat CGESpecialFilterE96::paramColorGradientValue[] = {0.0f,    8000.0f,  25000.0f, 0.0f, 32768.0f,
                                                                0.0f,    32768.0f, 32768.0f, 0.0f, 32768.0f,
                                                                8000.0f, 0.0f,     32768.0f, 0.0f, 0.0f};
...
void CGESpecialFilterE92::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    handler->setAsTarget();
    m_program.bind();
    ...
    // More initialization...
    {
        ...
        CGE_LOG_INFO("getHalfToneLowFac took %gs\n", (double)(clock() - t) / CLOCKS_PER_SEC);
...
bool CGESpecialFilterE73::initWithinCommonFilter(CGEMutipleEffectFilter* htProc)
{
    ...
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE73))
    {
        ...
        CGECurveInterface::CurvePoint pnt1B[3] = {{0.423529f, 0.243137f}, {0.690196f, 0.537255f}, {1.0f, 0.721569f}};
        ...
        CGECurveInterface::CurvePoint png2B[5] = {{0.211765f, 0.243137f},
                                                  {0.337255f, 0.396078f},
                                                  {0.505882f, 0.607843f},
                                                  {0.74902f, 0.752941f},
                                                  {1.0f, 0.870588f}};
...
bool CGESpecialFilterE74::initWithinCommonFilter(CGEMutipleEffectFilter* htProc)
{
    ...
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE74))
    {
        ...
        CGECurveInterface::CurvePoint pnt1R[3] = {{0.247059f, 0.137255f}, {0.580392f, 0.454902f}, {1.0f, 0.815686f}};
        CGECurveInterface::genCurve(vecR, pnt1R, 3);

        CGECurveInterface::CurvePoint pnt1G[3] = {{0.278431f, 0.0f}, {0.584314f, 0.329412f}, {1.0f, 0.733333f}};
        CGECurveInterface::genCurve(vecG, pnt1G, 3);

        CGECurveInterface::CurvePoint pnt1B[3] = {{0.333333f, 0.0862745f}, {0.694118f, 0.341176f}, {1.0f, 0.580392f}};
        CGECurveInterface::genCurve(vecB, pnt1B, 3);
        ...
        CGECurveInterface::CurvePoint pnt2R[5] = {{0.0196078f, 0.164706f},
                                                  {0.305882f, 0.505882f},
                                                  {0.458824f, 0.698039f},
                                                  {0.713726f, 0.858824f},
                                                  {0.988235f, 0.992157f}};
...
bool CGESpecialFilterE75::initWithinCommonFilter(CGEMutipleEffectFilter* htProc)
{
    ...
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE73))
    {
        ...
        CGECurveInterface::CurvePoint pnt1B[2] = {{0.0627451f, 0.0f}, {1.0f, 0.760784f}};
...
bool CGESpecialFilterE76::initWithinCommonFilter(CGEMutipleEffectFilter* htProc)
{
    ...
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE76))
    {
        ...
        CGECurveInterface::CurvePoint pnt1R[5] = {{0.0156863f, 0.137255f},
                                                  {0.278431f, 0.309804f},
                                                  {0.54902f, 0.627451f},
                                                  {0.698039f, 0.87451f},
                                                  {0.819608f, 1.0f}};
        ...
        CGECurveInterface::CurvePoint pnt1G[5] = {{0.0352941f, 0.0f},
                                                  {0.286275f, 0.239216f},
                                                  {0.54902f, 0.635294f},
                                                  {0.796078f, 0.878431f},
                                                  {0.972549f, 0.972549f}};
        ...
        CGECurveInterface::CurvePoint pnt1B[6] = {{0.0f, 0.121569f},      {0.431373f, 0.4f},
                                                  {0.568627f, 0.580392f}, {0.764706f, 0.717647f},
                                                  {0.913725f, 0.835294f}, {0.992157f, 0.92549f}};
        ...
        CGECurveInterface::CurvePoint pnt2R[5] = {{0.0196078f, 0.145098f},
                                                  {0.270588f, 0.333333f},
                                                  {0.466667f, 0.592157f},
                                                  {0.615686f, 0.835294f},
                                                  {0.815686f, 1.0f}};
        ...
        CGECurveInterface::CurvePoint pnt2G[5] = {{0.0509804f, 0.0196078f},
                                                  {0.290196f, 0.305882f},
                                                  {0.427451f, 0.564706f},
                                                  {0.611765f, 0.788235f},
                                                  {0.980392f, 0.980392f}};
        ...
        CGECurveInterface::CurvePoint png2B[6] = {{0.00784314f, 0.141176f}, {0.247059f, 0.286275f},
                                                  {0.364706f, 0.439216f},   {0.686275f, 0.745098f},
                                                  {0.945098f, 0.890196f},   {1.0f, 0.921569f}};
...
bool CGESpecialFilterE78::initWithinCommonFilter(CGEMutipleEffectFilter* htProc)
{
    ...
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE73))
    {
        ...
        CGECurveInterface::CurvePoint pnt1R[5] = {{0.0627451f, 0.0980392f},
                                                  {0.231373f, 0.301961f},
                                                  {0.396078f, 0.560784f},
                                                  {0.678431f, 0.827451f},
                                                  {1.0f, 0.929412f}};
        ...
        CGECurveInterface::CurvePoint pnt1G[5] = {{0.0941176f, 0.027451f},
                                                  {0.333333f, 0.286275f},
                                                  {0.486275f, 0.584314f},
                                                  {0.72549f, 0.811765f},
                                                  {0.976471f, 0.898039f}};
        ...
        CGECurveInterface::CurvePoint pnt1B[5] = {
            {0.117647f, 0.027451f}, {0.411765f, 0.305882f}, {0.588235f, 0.580392f},
            {0.784314f, 0.756863f}, {1.0f, 0.8f},
        };
        ...
        CGECurveInterface::CurvePoint pnt2R[5] = {{0.0627451f, 0.156863f},
                                                  {0.203922f, 0.435294f},
                                                  {0.486275f, 0.760784f},
                                                  {0.67451f, 0.878431f},
                                                  {0.905882f, 0.988235f}};
        ...
        CGECurveInterface::CurvePoint pnt2G[5] = {{0.0745098f, 0.0196078f},
                                                  {0.278431f, 0.329412f},
                                                  {0.392157f, 0.568627f},
                                                  {0.654902f, 0.831373f},
                                                  {0.870588f, 0.980392f}};
...
bool CGESpecialFilterE79::initWithinCommonFilter(CGEMutipleEffectFilter* htProc)
{
    ...
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE79))
    {
        ...
        CGECurveInterface::CurvePoint pnt1G[3] = {{0.239216f, 0.0862745f}, {0.639216f, 0.52549f}, {1.0f, 0.784314f}};
        CGECurveInterface::genCurve(vecG, pnt1G, 3);

        CGECurveInterface::CurvePoint pnt1B[3] = {{0.282353f, 0.0117647f}, {0.764706f, 0.52549f}, {1.0f, 0.690196f}};
        CGECurveInterface::genCurve(vecB, pnt1B, 3);
        ...
        CGECurveInterface::CurvePoint pnt2R[5] = {{0.0392157f, 0.168627f},
                                                  {0.243137f, 0.439216f},
                                                  {0.45098f, 0.690196f},
                                                  {0.643137f, 0.85098f},
                                                  {1.0f, 0.952941f}};
        ...
        CGECurveInterface::CurvePoint pnt2G[6] = {
            {0.0117647f, 0.109804f}, {0.207843f, 0.380392f}, {0.32549f, 0.568627f},
            {0.427451f, 0.721569f},  {0.705882f, 0.905882f}, {0.94902f, 0.988235f},
        };
        CGECurveInterface::genCurve(vecG, pnt2G, 6);

        CGECurveInterface::CurvePoint png2B[6] = {{0.0313726f, 0.0313726f}, {0.270588f, 0.4f},
                                                  {0.388235f, 0.572549f},   {0.65098f, 0.870588f},
                                                  {0.768627f, 0.921569f},   {1.0f, 0.945098f}};
        CGECurveInterface::genCurve(vecB, png2B, 6);
...
bool CGESpecialFilterE80::initWithinCommonFilter(CGEMutipleEffectFilter* htProc)
{
    ...
    if (initShadersFromString(g_vshDefaultWithoutTexCoord, s_fshE73))
    {
        ...
        CGECurveInterface::CurvePoint pnt1G[3] = {{0.301961f, 0.0f}, {0.560784f, 0.270588f}, {1.0f, 0.690196f}};
        CGECurveInterface::genCurve(vecG, pnt1G, 3);

        CGECurveInterface::CurvePoint pnt1B[3] = {{0.176471f, 0.0f}, {0.623529f, 0.380392f}, {1.0f, 0.678431f}};
        CGECurveInterface::genCurve(vecB, pnt1B, 3);
        ...
        CGECurveInterface::CurvePoint pnt2R[3] = {{0.027451f, 0.670588f}, {0.360784f, 0.788235f}, {1.0f, 0.992157f}};
        ...
        CGECurveInterface::CurvePoint png2B[5] = {{0.0f, 0.0901961f},
                                                  {0.207843f, 0.341176f},
                                                  {0.403922f, 0.54902f},
                                                  {0.729412f, 0.815686f},
                                                  {0.937255f, 0.972549f}};
