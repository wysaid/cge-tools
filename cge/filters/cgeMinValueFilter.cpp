...
static CGEConstString s_minValueFunc = CGE_SHADER_STRING(
    ...
    float lum(vec4 value) { return dot(value.rgb, vec3(0.299, 0.587, 0.114)); } vec4 getValue(
        vec4 newValue, vec4 originValue) { return mix(newValue, originValue, step(lum(originValue), lum(newValue))); });

static CGEConstString s_vshMinValue3x3 = CGE_SHADER_STRING(

    varying vec2 texCoord[9]; attribute vec2 vPosition;
...
static CGEConstString s_vshMinValue3x3_2 = CGE_SHADER_STRING(
    ...
    // Compatibility for OpenGL ES devices that may support only 8 varyings
    varying vec4 texCoord0; varying vec4 texCoord1; varying vec4 texCoord2; varying vec4 texCoord3;
    varying vec2 texCoord4; attribute vec2 vPosition;
...
static CGEConstString s_fshMinValue3x3 =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 texCoord[9];
...
static CGEConstString s_fshMinValue3x3_2 =
    CGE_SHADER_STRING_PRECISION_M(varying vec4 texCoord0; varying vec4 texCoord1; varying vec4 texCoord2;
                                  varying vec4 texCoord3; varying vec2 texCoord4;
...
bool CGEMinValueFilter3x3::init()
{
    ...
    if (!initShadersFromString(s_vshMinValue3x3, buffer))
    {
        ...
        if (!initShadersFromString(s_vshMinValue3x3_2, buffer)) return false;
    }
...
static CGEConstString s_vshMinValue3x3Plus =
    CGE_SHADER_STRING(varying vec2 texCoord[13];
...
static CGEConstString s_vshMinValue3x3Plus_2 =
    CGE_SHADER_STRING(varying vec4 texCoord0; varying vec4 texCoord1; varying vec4 texCoord2; varying vec4 texCoord3;
                      varying vec4 texCoord4; varying vec4 texCoord5; varying vec2 texCoord6;
...
static CGEConstString s_fshMinValue3x3Plus =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 texCoord[13];
...
bool CGEMinValueFilter3x3Plus::init()
{
    ...
    if (!initShadersFromString(s_vshMinValue3x3Plus, buffer))
    {
        ...
        if (!initShadersFromString(s_vshMinValue3x3Plus_2, buffer)) return false;
    }
