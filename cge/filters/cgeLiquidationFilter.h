...
public:
    ...
    //'stride' range: (0.001, 0.2]. The mesh would be larger as stride is smaller and the result would be more accurate
    //but may take longer time for processing.
    ...
    virtual void forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
                                   float intensity);  // Forward warp tool
    virtual void pushLeftDeformMesh(
        Vec2f start, Vec2f end, float w, float h, float radius, float intensity,
        float angle = 3.14159f /
            2.0f);  //(Left) push tool: pushes mesh along the path in the given angle direction (left side by default).
                    //The angle parameter defaults to left and can be set to any value.
    virtual void restoreMeshWithPoint(Vec2f pnt, float w, float h, float radius, float intensity);  // Reconstruct tool
    virtual void bloatMeshWithPoint(Vec2f pnt, float w, float h, float radius, float intensity);  // Bloat (expand) tool
    virtual void wrinkleMeshWithPoint(Vec2f pnt, float w, float h, float radius,
                                      float intensity);  // Wrinkle (pucker) tool
...
{
public:
    void forwardDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius,
                           float intensity);  // see in 'CGELiquidationFilter'
    virtual void pushLeftDeformMesh(Vec2f start, Vec2f end, float w, float h, float radius, float intensity,
                                    float angle = 3.14159f / 2.0f);  // see in 'CGELiquidationFilter'
};
