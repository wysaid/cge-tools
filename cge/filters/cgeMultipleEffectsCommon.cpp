...
void getScale(GLfloat* colorScaleLow, GLfloat* colorScaleRange, GLfloat* sat, GLfloat dark, GLfloat light, GLint w,
              GLint h)
{
...
void getHalfToneLowFac(GLfloat* low, GLfloat* fac, GLint w, GLint h)
{
    ...
    for (int i = 0; i != total; ++i)
    {
        ++lum[(unsigned char)((data[0] * int(256 * 0.299) + data[1] * int(256 * 0.587) + data[2] * int(256 * 0.114)) >>
                              8)];
        data += 4;
    }
...
static CGEConstString s_fshLomo =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
                                  ...
                                  uniform float intensity; uniform float saturation;
...
static CGEConstString s_fshLomoLinear =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
...
static CGEConstString s_fshLomoWithCurve = CGE_SHADER_STRING_PRECISION_M(
    ...
    void main() {
        ...
        gl_FragColor = vec4(curveArray[int(dst.r * curvePrecision)].r, curveArray[int(dst.g * curvePrecision)].g,
                            curveArray[int(dst.b * curvePrecision)].b, src.a);
    });
...
static CGEConstString s_fshLomoWithCurveTex = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform sampler2D curveTexture;
    ...
    void main() {
        ...
        gl_FragColor = vec4(texture2D(curveTexture, vec2(dst.r, 0.0)).r, texture2D(curveTexture, vec2(dst.g, 0.0)).g,
                            texture2D(curveTexture, vec2(dst.b, 0.0)).b, src.a);
    });
...
void CGELomoFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture, GLuint vertexBufferID)
{
    ...
    // More initialization...
    {
        ...
        if (s_colorScaleStatus > 0 && m_scaleDark >= 0.0f && m_scaleLight > 0.0f)
        {
            ...
            CGE_LOG_INFO("getscale took %gs\n", (double)(clock() - t) / CLOCKS_PER_SEC);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, handler->getTargetTextureID(),
                                   0);
        }
...
void CGELomoWithCurveTexFilter::flush()
{
    ...
    {
        m_curveTexture =
            cgeGenTextureWithBuffer(samplerData, int(sz), 1, GL_RGB, dataType, 3, 0, GL_NEAREST, GL_CLAMP_TO_EDGE);
    }
    else
    {
...
static CGEConstString s_fshColorScale =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture;
...
static CGEConstString s_fshColorMul =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform %s cmul;
