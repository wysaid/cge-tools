...
const static char* const s_fshFastBlur = CGE_SHADER_STRING_PRECISION_H(
    ...
    uniform int blurSamplerScale; uniform vec2 blurNorm; uniform int samplerRadius;
...
const static char* const s_fshFixedRadiusBlur = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 samplerSteps;
    uniform int blurSamplerScale; uniform vec2 blurNorm; const int samplerRadius = 4;
...
const static char* const s_fshSharpenBlur_TwoStep = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 samplerSteps;
    uniform int blurSamplerScale; uniform vec2 blurNorm; uniform float intensity; uniform int samplerRadius;
...
static CGEConstString s_fshSharpenBlur_Simple = CGE_SHADER_STRING_PRECISION_H(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec2 samplerSteps;
    uniform float blurSamplerScale; uniform float intensity;
...
static CGEConstString s_fshSharpenBlur_SimpleBetter = CGE_SHADER_STRING_PRECISION_H(
    ...
    void main() {
        ...
        tmp += texture2D(inputImageTexture, textureCoordinate - blurSamplerScale * samplerSteps);
        tmp +=
            texture2D(inputImageTexture, textureCoordinate + blurSamplerScale * vec2(-samplerSteps.x, samplerSteps.y));
        tmp +=
            texture2D(inputImageTexture, textureCoordinate + blurSamplerScale * vec2(samplerSteps.x, -samplerSteps.y));
...
{
    if (m_samplerScaleValue == value) return;
...
void CGESharpenBlurFastFilter::render2Texture(CGEImageHandlerInterface* handler, GLuint srcTexture,
