...
static CGEConstString s_fshWrinkleRemove = CGE_SHADER_STRING_PRECISION_M(
    ...
    uniform float samplerScale; uniform vec2 noiseRange; uniform vec2 samplerSteps; uniform float noiseIntensity;

    const int samplerRadius = 5;

    float random(vec2 seed) { return fract(sin(dot(seed, vec2(12.9898, 78.233))) * 43758.5453); }

    vec3 screen(vec3 src1, vec3 src2, float alpha) { return mix(src1, src1 + src2 - src1 * src2, alpha); }
