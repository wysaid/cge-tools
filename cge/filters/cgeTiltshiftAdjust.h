...
public:
    ...
    // You should know that the sampling would be very slow in OpenGL ES2.0+ on your device if the sampling-radius is
    // too large. The radius you set by "setBlurRadiusScale" simply provides a sampling radius scale whose true radius
    // is less than the "limit" set by this function. This should be called before setBlurRadiusScale
...
public:
    ...
    // gradient > 1. The gradient is defined as the multiples of "RadiusStart".And ("BlurGradient" - "RadiusStart") is
    // the gradual changing zone.
