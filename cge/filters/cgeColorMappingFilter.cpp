...
static CGEConstString s_vshMapingBuffered =
    CGE_SHADER_STRING(attribute vec2 vPosition; attribute vec2 vTexPosition; varying vec2 textureCoordinate;
...
static CGEConstString s_fshMapingBuffered =
    CGE_SHADER_STRING_PRECISION_M(varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; void main() {
...
public:
    ...
    {
        assert(m_mappingAreas.size() > 0 &&
               m_cacheBufferData.size() >= m_texUnitResolution.width * m_texUnitResolution.height &&
               m_texVertBufferData.size() >= m_texUnitResolution.width * m_texUnitResolution.height * 6);
        ...
        glFinish();
        glReadPixels(0, 0, m_texUnitResolution.width, m_texUnitResolution.height, GL_RGBA, GL_UNSIGNED_BYTE,
                     m_cacheBufferData.data());
        ...
        glBufferSubData(GL_ARRAY_BUFFER, 0, m_texVertBufferData.size() * sizeof(m_texVertBufferData[0]),
                        m_texVertBufferData.data());
    ...
    void setupMapping(GLuint mappingTex, int texWidth, int texHeight, int texResoWidth, int texResoHeight)
    {
        CGEColorMappingFilter::setupMapping(mappingTex, texWidth, texHeight, texResoWidth, texResoHeight);

        m_cacheBufferData.resize(texResoWidth * texResoHeight);
        m_texVertBufferData.resize(texResoWidth * texResoHeight * 6);

        if (m_mappingVertBuffer == 0) glGenBuffers(1, &m_mappingVertBuffer);

        if (m_mappingTexVertBuffer == 0) glGenBuffers(1, &m_mappingTexVertBuffer);

        CGESizef mappingSize(m_texSize.width / (float)m_texUnitResolution.width,
                             m_texSize.height / (float)m_texUnitResolution.height);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, m_mappingTexVertBuffer);
        glBufferData(GL_ARRAY_BUFFER, m_texVertBufferData.size() * sizeof(m_texVertBufferData[0]), nullptr,
                     GL_STREAM_DRAW);
    }
...
void CGEColorMappingFilter::setupMapping(GLuint mappingTex, int texWidth, int texHeight, int texResoWidth,
                                         int texResoHeight)
{
    m_mappingTexture = mappingTex;
    m_texSize.set(texWidth, texHeight);
    m_texUnitResolution.set(texResoWidth, texResoHeight);
}
