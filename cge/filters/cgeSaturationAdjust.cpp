...
const static char* const s_fshSaturation = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform float saturation; uniform float hue;
...
const static char* const s_fshSaturationFast = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform float intensity; void main() {
        vec4 src = texture2D(inputImageTexture, textureCoordinate);
...
const static char* const s_fshHSVAdjust = CGE_SHADER_STRING_PRECISION_M(
    varying vec2 textureCoordinate; uniform sampler2D inputImageTexture; uniform vec3 vColor1; uniform vec3 vColor2;
...
void CGESaturationHSVFilter::setAdjustColors(float red, float green, float blue, float magenta, float yellow,
                                             float cyan)
{
    m_program.bind();
